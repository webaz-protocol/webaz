...
export function registerDisputesReadRoutes(app: Application, deps: DisputesReadDeps): void {
  ...
  app.get('/api/disputes/:id', async (req, res) => {
    ...
    const lastDispFrom = await dbOne<{ from_status: string }>("SELECT from_status FROM order_state_history WHERE order_id = ? AND to_status = 'disputed' ORDER BY created_at DESC LIMIT 1", [dispute.order_id])
