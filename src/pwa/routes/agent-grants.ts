...
import { chooseSubmitPaymentOption, paymentOptionsForSubmitRequest } from '../order-submit-choose-payment.js'  // RFC-029 Design A PR-3 choice/update contract
...
export function registerAgentGrantsRoutes(app: Application, deps: AgentGrantsDeps): void {
  ...
  // RFC-029 Design A PR-3 — 支付方式选择(§Choice/update contract)。人类本人在确认页为 deferred 请求选轨道。
  //   GET: 列当前可选支付方式(卖家支持+资格通过)。POST: 选定 → 写草稿 rail/account + 重算 params_hash(旧 Passkey 票据即失效)。
  app.get('/api/agent-grants/permission-requests/:id/payment-options', (req, res) => {
    const user = auth(req, res); if (!user) return
    const r = paymentOptionsForSubmitRequest(db, { requestId: req.params.id, humanId: user.id as string, nowIso: new Date().toISOString(), getProtocolParam })
    if (!r.ok) return void res.status(r.http).json({ error: r.error, error_code: r.error_code })
    res.json({ request_id: r.request_id, rail_chosen: r.rail_chosen, current_rail: r.current_rail, options: r.options })
  })
  app.post('/api/agent-grants/permission-requests/:id/choose-payment', (req, res) => {
    const user = auth(req, res); if (!user) return
    const r = chooseSubmitPaymentOption(db, { requestId: req.params.id, humanId: user.id as string, optionId: String((req.body ?? {}).option_id ?? ''), nowIso: new Date().toISOString(), deps: { generateId, getProtocolParam }, getProtocolParam })
    if (!r.ok) return void res.status(r.http).json({ error: r.error, error_code: r.error_code })
    res.json({ success: true, request_id: r.request_id, payment_rail: r.payment_rail, params_hash: r.params_hash, approval_url: `/#agent-approvals/${r.request_id}`, note: 'Payment method set — now approve with your Passkey. Any Passkey token minted before this choice is invalidated (the request hash changed).' })
  })
