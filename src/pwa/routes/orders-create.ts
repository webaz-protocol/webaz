...
import { createDirectPayResponse } from '../../direct-pay-create.js'; import { isDeferredRail } from '../../direct-pay-rails.js'; import { wazEscrowChannelOn, WAZ_RAIL_DISABLED } from '../../waz-escrow-channel.js'; import { createUsdcEscrowResponse } from '../../usdc-escrow-create.js'   // PR-4c direct_p2p 建单分叉 + RFC-029 Design A 纵深防御('deferred' 绝不建单)
...
export function registerOrdersCreateRoutes(app: Application, deps: OrdersCreateDeps): void {
  ...
  app.post('/api/orders', async (req, res) => {
    ...
    if (String(req.body?.payment_rail || '') === 'usdc_escrow') return void createUsdcEscrowResponse(res, db, { generateId, appendOrderEvent, getProtocolParam }, { product, buyerId: user.id as string, reqQty, basePrice, totalAmount, totalAmountU, shippingAddress, agentApiKey: apiKey, draftId: _dl.kind === 'link' ? _dl.draftId : undefined, consumePriceSession: () => consumePriceSession(db, typeof session_token === 'string' ? session_token : undefined), opts: { variantId: variant_id, hasVariants: Number(product.has_variants) === 1, flashActive: !!flashSale, couponCode: coupon_code, buyInsurance: !!buy_insurance, donationPct: donationPctNum, isGift: !!is_gift, anonymous: anonymousFlag === 1, deliveryWindow: !!delivery_window }, shipping: { region: _ship.region, fee: _ship.fee, estDays: _ship.estDays, quoteRequired: _ship.quoteRequired } }); if (!wazEscrowChannelOn(getProtocolParam)) return void res.status(409).json(WAZ_RAIL_DISABLED)   // ↑B3 USDC 合约担保分叉(本金进链上合约,零 wallets 写)| WAZ 退役硬闸(默认关,fail-closed):此行以下全是 escrow 建单路径,堵直接 POST 与省略 rail 的一切 fallthrough;菜单层同真值
    ...
    try {
      db.transaction(() => {
        ...
        consumePriceSession(db, typeof session_token === 'string' ? session_token : undefined)
      ...
      if (e instanceof AgentSpendCapExceeded) return void res.status(403).json(e.violation); if (e instanceof PriceSessionConsumeError) return void res.status(409).json({ error: e.message, error_code: 'PRICE_SESSION_USED' }); const msg = (e as Error).message
