...
export function registerOrdersActionRoutes(app: Application, deps: OrdersActionDeps): void {
  ...
  app.post('/api/orders/:id/confirm-in-person', async (req, res) => {
    ...
    if (order.payment_rail === 'usdc_escrow') return void res.status(409).json({ error: 'USDC 担保订单请在订单页用你的链上钱包完成「确认收货并放款」,无需此操作', error_code: 'USDC_ESCROW_CONFIRM_NOT_WIRED' })   // B3 R2:面交完成也是 confirm→settle 路径,同一硬闸(settleOrder 对本轨已 fail-closed throw,这里给友好 409)
  ...
  app.post('/api/orders/:id/action', async (req, res) => {
    ...
    if (action === 'dispute_withdraw_confirm') {
      ...
      if (order.payment_rail === 'usdc_escrow') return void res.status(409).json({ error: 'USDC 担保订单请在订单页用你的链上钱包完成「确认收货并放款」,无需此操作', error_code: 'USDC_ESCROW_CONFIRM_NOT_WIRED' })   // B3 R2:撤诉确认=confirm→settle 同路,同一硬闸
    ...
    if (toStatus === 'confirmed' && order.payment_rail === 'usdc_escrow') {
      ...
      return void res.status(409).json({ error: 'USDC 担保订单请在订单页用你的链上钱包完成「确认收货并放款」,无需此操作', error_code: 'USDC_ESCROW_CONFIRM_NOT_WIRED' })
    }
