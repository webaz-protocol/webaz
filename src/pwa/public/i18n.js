...
const _EN = {
  ...
  '无法安全批准 —— 关键条款不完整(金额/币种/支付轨道/收款账户)。请稍后重试,或让 agent 重新报价并建草稿。': 'Cannot approve safely — key terms incomplete (amount / currency / payment rail / receiving account). Retry later, or have the agent re-quote and re-draft.',
  '卖家未配置可用的直付收款目的地 —— 已禁止批准': 'Seller has no usable direct-pay receiving destination — approval is blocked.',
  '卖家收款目的地': 'Seller receiving destination',
  '卖家默认收款账户': 'Seller default receiving account',
  ...
  '一个 AI 客户端请求通过 OAuth 连接你的 WebAZ 账号。它只会拿到下列受限、可随时撤销的权限 —— 不是你的账号或密钥;资金/发布/发货等敏感动作永远需要你的 Passkey 逐次批准。你可选择连接保持多久,期间客户端会自动续期,无需反复重连。': 'An AI client is requesting to connect to your WebAZ account via OAuth. It will only receive the limited, revocable permissions below — never your account or keys; money, publishing, shipping and other sensitive actions always require your Passkey per action. You choose how long the connection lasts; the client renews automatically during that window, so you will not have to reconnect repeatedly.',
  ...
  '连接保持时长': 'Keep this connection for',
  '1 天': '1 day',
  '30 天(推荐)': '30 days (recommended)',
  '到期后需重新授权;你也可随时在「已连接的应用」中撤销。': 'You will re-authorize after it expires; you can also revoke it anytime under “Connected apps”.',
  ...
  '你在该卖家的进行中 USDC 担保订单已达上限': 'You have reached the open USDC escrow order cap with this seller',
  'USDC 担保暂仅支持无规格商品': 'USDC escrow currently supports simple (no-variant) products only',
  ...
  '🔌 已连接的应用与 Agent': '🔌 Connected apps & agents',
  '尚无已连接的应用或 Agent': 'No connected apps or agents yet',
  'AI 应用通过 OAuth 连接、或 agent 通过 webaz_pair 配对,经你 Passkey 批准后出现在这里': 'AI apps that connect via OAuth, or agents paired via webaz_pair, appear here after your Passkey approval',
  '这些是你授权给 AI 应用/agent 的连接凭证（作用域受限、可随时撤销）。它们不是你的账号或密钥，永远无法动用资金、投票或改密钥；撤销后其访问令牌立即失效。': 'These are scoped, revocable connection grants you authorized for AI apps/agents. They are not your account or keys and can never move funds, vote, or change keys; revoking one immediately invalidates its access tokens.',
  '有效': 'Active',
  '最近使用': 'Last used',
  '从未使用': 'Never used',
  '有效期至': 'Expires',
  ...
  '正在载入支付方式…': 'Loading payment methods…',
  '支付方式载入失败,请刷新重试': 'Failed to load payment methods — refresh and retry',
  '该卖家当前无可用支付方式': 'This seller has no available payment method right now',
  '选择支付方式(将在你 Passkey 批准时确定)': 'Choose a payment method (fixed when you approve with Passkey)',
  '托管(模拟测试轨)': 'Escrow (simulated test rail)',
  '推荐支付方式': 'Recommended',
  '确定支付方式并用 Passkey 批准': 'Confirm payment method & approve with Passkey',
  '请先选择一种支付方式': 'Please choose a payment method first',
  '支付方式选择失败,请刷新重试': 'Could not set the payment method — refresh and retry',
  ...
  '订单已超时': 'Order expired',
  '已关单': 'Closed',
  '系统已按协议处置并关闭订单': 'Closed by protocol disposal',
  ...
  '链上冻结失败,请稍后重试或核对链上状态': 'On-chain freeze failed — please retry or reconcile the on-chain state',
  ...
  '资金已入合约但订单已取消': 'Funds in contract but order cancelled',
  ...
  // ── 2026-07 订单流遍历审计:自动执法/裁定/undeliverable/直付取消/协商取消 通知模板句对 ──
  // (「订单已取消」「争议已驳回」已有存量键,勿重复)
  '订单「{product}」已取消(付款前)。': 'Order "{product}" was cancelled (before payment).',
  '物流方超时未揽收,已自动记录违约。': 'The carrier failed to pick up in time; the default was recorded automatically.',
  '物流方超时未投递,已自动记录违约。': 'The carrier failed to deliver in time; the default was recorded automatically.',
  '卖家违约处置完成,订单已关闭': 'Seller default settled — order closed',
  '订单「{product}」违约处置完成:{amount} WAZ 已全额退回买家,卖家质押已按违约罚没。': 'Order "{product}" default settled: {amount} WAZ fully refunded to the buyer; the seller stake was forfeited per the default rules.',
  '订单「{product}」已按卖家违约关闭。直付非托管:平台不持货款、无平台退款 —— 如你已场外付款,请通过订单聊天与卖家协商退款;卖家违约已记入信誉。': 'Order "{product}" was closed as a seller default. Direct Pay is non-custodial: the platform holds no funds and cannot refund — if you already paid off-protocol, use the order chat to negotiate the refund with the seller; the default is recorded on their reputation.',
  '物流违约处置完成,订单已关闭': 'Logistics default settled — order closed',
  '订单「{product}」物流违约处置完成,已按协议从物流质押赔付/退款。': 'Order "{product}" logistics default settled; compensation/refund was paid from the logistics stake per protocol.',
  '订单「{product}」已按物流违约关闭(非托管:不涉平台资金;违约已记录)。': 'Order "{product}" was closed as a logistics default (non-custodial: no platform funds involved; the default is recorded).',
  '买家责任处置完成,订单已关闭': 'Buyer fault settled — order closed',
  '订单「{product}」买家责任处置完成,资金已按协议结算给卖家。': 'Order "{product}" buyer-fault settled; funds were settled to the seller per protocol.',
  '订单「{product}」已按买家责任关闭(非托管:不涉平台资金,仅信誉记录)。': 'Order "{product}" was closed as buyer fault (non-custodial: no funds involved, reputation record only).',
  '卖家举证未派送成功': 'Seller evidenced failed delivery',
  '订单「{product}」:卖家/物流举证包裹按订单地址投递被退回/拒收。如你认为举证不实,请在窗口内发起争议;逾期未争议将落定为买家责任。': 'Order "{product}": the seller/carrier evidenced the parcel was returned/refused after delivery to the order address. If you believe this is inaccurate, open a dispute within the window; otherwise buyer fault will be established.',
  '未派送成功责任落定': 'Failed-delivery fault established',
  '订单「{product}」:买家未在窗口内争议,未派送成功责任落定为买家。': 'Order "{product}": the buyer did not contest within the window; failed-delivery fault is established as the buyer\'s.',
  '订单「{product}」:买家未在窗口内争议,责任落定为买家,订单关闭(非托管:仅信誉记录,不涉资金)。': 'Order "{product}": the buyer did not contest within the window; buyer fault established and the order closed (non-custodial: reputation record only, no funds).',
  '订单「{product}」责任已落定,托管资金保持锁定等待货物退回:卖家确认收到退货后按成本扣除结算;卖家逾期未确认将默认全款退回买家。': 'Order "{product}": fault established; escrow stays locked pending goods return — once the seller confirms receipt, settlement deducts return costs; if the seller misses the window, the buyer gets a full refund by default.',
  '退货流程已结算': 'Return flow settled',
  '订单「{product}」退货流程收口,托管资金已按协议结算(卖家确认收货=扣除退程成本后退款;卖家逾期未确认=默认全款退回买家)。': 'Order "{product}" return flow closed; escrow settled per protocol (seller confirmed receipt = refund minus return costs; seller missed the window = full refund to the buyer by default).',
  '订单「{product}」争议已裁定卖家胜诉,资金已释放给卖家。': 'The dispute on order "{product}" was ruled for the seller; funds released to the seller.',
  '订单「{product}」争议已裁定卖家胜诉(直付为信誉裁决,不涉资金流转)。': 'The dispute on order "{product}" was ruled for the seller (Direct Pay rulings are reputation-only — no funds move).',
  '争议裁定:部分退款': 'Dispute ruled: partial refund',
  '订单「{product}」争议已裁定,已按裁定部分退款给买家。': 'The dispute on order "{product}" was ruled; a partial refund was issued to the buyer per the ruling.',
  '订单「{product}」争议已裁定部分责任(直付为信誉裁决,非托管不涉平台退款)。': 'The dispute on order "{product}" was ruled partial liability (Direct Pay rulings are reputation-only; non-custodial, no platform refund).',
  '订单「{product}」争议已裁定全额退款,{amount} WAZ 已退回买家。': 'The dispute on order "{product}" was ruled a full refund; {amount} WAZ returned to the buyer.',
  '订单「{product}」的争议被驳回(无效),订单维持原结论。': 'The dispute on order "{product}" was dismissed (invalid); the order outcome stands.',
  '直付订单已取消': 'Direct Pay order cancelled',
  '买家在付款前取消了订单「{product}」,平台费质押已释放,库存已回补。': 'The buyer cancelled order "{product}" before paying; the fee stake was released and stock restored.',
  '直付订单已关闭': 'Direct Pay order closed',
  '订单「{product}」已关闭(付款窗口超时,买家确认未付款)。': 'Order "{product}" was closed (payment window expired; buyer confirmed not paid).',
  '对方提议协商取消': 'Counterparty proposed mutual cancellation',
  '订单「{product}」:对方提议无责协商取消,请到订单页处理(同意/拒绝)。': 'Order "{product}": the other party proposed a no-fault mutual cancellation — go to the order page to accept or decline.',
  '协商取消达成,订单已关闭': 'Mutual cancellation agreed — order closed',
  '订单「{product}」双方协商一致无责取消:货款已全额退回买家,卖家质押已退还,双方信誉不受影响。': 'Order "{product}" was mutually cancelled with no fault: payment fully refunded to the buyer, seller stake returned, neither reputation affected.',
  '订单「{product}」双方协商一致无责取消(非托管:零资金操作,场外款项以双方约定为准),双方信誉不受影响。': 'Order "{product}" was mutually cancelled with no fault (non-custodial: zero funds moved; off-protocol payments settle per your agreement). Neither reputation is affected.',
  ...
  '买家申索成立(卖家未退款,信誉已追加处罚)': 'Buyer claim upheld (seller did not refund; extra reputation penalty applied)',
  '卖家退款成立(申索不成立)': 'Seller refund confirmed (claim dismissed)',
  '退款申索仲裁进行中(信誉裁决),可在争议页补充证据。': 'Refund-claim arbitration in progress (reputation ruling); add evidence on the dispute page.',
  '请先核实退款已到账;未收到请勿确认,可举证升级仲裁。': 'Verify the refund arrived first; if not received, do NOT confirm — you can escalate with evidence.',
  '已收到退款,确认留档(需 Passkey)': 'Refund received — confirm & record (Passkey)',
  '未收到,举证仲裁': 'Not received — escalate with evidence',
  '已向卖家发出退款请求,等待响应(超期可举证仲裁)。': 'Refund request sent to the seller; awaiting response (escalate with evidence if it expires).',
  '卖家拒绝了退款请求。': 'The seller declined the refund request.',
  '卖家未在期限内响应。': 'The seller did not respond within the window.',
  '举证升级仲裁': 'Escalate with evidence',
  '说明(可选,卖家可见):付款方式/时间/金额': 'Note (optional, visible to seller): payment method / time / amount',
  '申请场外退款': 'Request off-protocol refund',
  '退款握手已闭环:你已确认收到卖家场外退款(已留档)。': 'Refund handshake closed: you confirmed receiving the seller\'s off-protocol refund (recorded).',
  '买家申请违约关单退款。': 'The buyer requested a refund for this default-closed order.',
  '已退款:附参考并点"我已退款";未退款:请尽快在协议外退款。持续不响应买家可举证仲裁,将追加信誉处罚。': 'Already refunded: add a reference and tap "I refunded". Not yet: refund off-protocol soon. Staying silent lets the buyer escalate to arbitration with an extra reputation penalty.',
  '你已声明退款,等待买家确认收到留档。': 'You declared the refund; awaiting buyer confirmation for the record.',
  '买家已确认收到退款,握手闭环留档。': 'The buyer confirmed receiving the refund; the handshake is closed and recorded.',
  '已发出退款请求,等待卖家响应': 'Refund request sent; awaiting seller response',
  '确认你已在协议外向买家完成退款?虚假声明将留痕并在仲裁中被追责。': 'Confirm you refunded the buyer off-protocol? A false declaration is recorded and held against you in arbitration.',
  '确认已收到卖家场外退款?确认后握手闭环留档,不可撤销。': 'Confirm you received the seller\'s off-protocol refund? This closes and records the handshake irreversibly.',
  '已收到退款': 'Refund received',
  '已确认收到退款,握手闭环': 'Refund receipt confirmed; handshake closed',
  '请说明情况(≥10 字):付款方式/时间/金额,以及卖家未退款的经过。提交后进入仲裁(信誉裁决),可在争议页补充凭证。': 'Describe the situation (≥10 chars): payment method/time/amount and how the seller failed to refund. Submitting opens arbitration (reputation ruling); add evidence on the dispute page.',
  '请说明情况(≥10 字):付款方式/时间/金额,以及卖家未退款的经过。': 'Describe the situation (≥10 chars): payment method/time/amount and how the seller failed to refund.',
  '退款申索已提交仲裁': 'Refund claim submitted to arbitration',
  '买家申请违约关单退款': 'Buyer requested a default-closure refund',
  '订单「{product}」因你违约被系统关闭,买家已场外付款并申请退款。已退款请点"我已退款"并附参考;未退款请尽快场外退款。持续不响应买家可举证仲裁,将追加信誉处罚。': 'Order "{product}" was closed by the system as your default; the buyer paid off-protocol and requested a refund. If refunded, tap "I refunded" with a reference; if not, refund off-protocol soon. Staying silent lets the buyer escalate to arbitration with an extra reputation penalty.',
  '卖家拒绝了退款握手': 'Seller declined the refund handshake',
  '订单「{product}」:卖家拒绝了退款握手请求。你可以举证升级仲裁(提供付款凭证,信誉裁决)。': 'Order "{product}": the seller declined the refund handshake. You can escalate to arbitration with payment evidence (reputation ruling).',
  '订单「{product}」:卖家声明已在协议外向你退款。请核实到账后确认(需 Passkey);未收到请勿确认,可举证升级仲裁。': 'Order "{product}": the seller declared an off-protocol refund to you. Verify it arrived, then confirm (Passkey); if not received, do NOT confirm — escalate with evidence.',
  '退款握手完成': 'Refund handshake completed',
  '订单「{product}」:场外退款握手已闭环留档(非托管,零资金操作)。': 'Order "{product}": the off-protocol refund handshake is closed and recorded (non-custodial; zero funds moved).',
  '买家撤回了退款握手请求': 'Buyer withdrew the refund handshake request',
  '订单「{product}」的退款握手请求已被买家撤回。': 'The refund handshake request on order "{product}" was withdrawn by the buyer.',
  '退款申索已进入仲裁': 'Refund claim entered arbitration',
  '订单「{product}」的退款申索已进入统一仲裁台(信誉裁决,非托管不涉资金)。当事方可在争议页提交/补充凭证。': 'The refund claim on order "{product}" entered the unified arbitration bench (reputation ruling; non-custodial, no funds involved). Parties can submit evidence on the dispute page.',
  '退款申索裁定:买家申索成立': 'Refund claim ruled: buyer upheld',
  '订单「{product}」退款申索已裁定:卖家未场外退款成立。卖家信誉已追加处罚并留公开违约记录(非托管:协议不代退,退款仍须双方场外完成)。': 'The refund claim on order "{product}" was ruled: the seller failed to refund off-protocol. An extra reputation penalty and a public default record were applied (non-custodial: the protocol cannot refund — settlement still happens off-protocol).',
  '退款申索裁定:卖家退款成立': 'Refund claim ruled: seller refund confirmed',
  '订单「{product}」退款申索已裁定:卖家已退款成立,申索不成立(发起方信誉按争议败诉记录)。': 'The refund claim on order "{product}" was ruled: the seller\'s refund was confirmed and the claim dismissed (the initiator takes a dispute-lost reputation record).',
  ...
  // PR-B6b-2:买家链上钱包面(EIP-1193 基础层 / 存入 stepper / 释放·争议)。
  // 诚实纪律:文案只描述【今天代码真的会做的事】—— 链上仲裁裁决(B7)未接线,争议文案绝不承诺可裁决。
  ...
  '网络': 'Network',
  '担保合约': 'Escrow contract',
  '卖家收款地址': 'Seller payout address',
  '平台费率': 'Platform fee rate',
  '自动放款时间': 'Auto-release time',
  ...
  '切换网络后继续': 'Switch network, then continue',
  '未检测到链上钱包': 'No on-chain wallet detected',
  '本轨需要浏览器里的链上钱包(如 MetaMask)。请安装后刷新本页,或改用支持链上钱包的浏览器。在此之前请不要向任何地址手动转账。': 'This rail needs an in-browser on-chain wallet (e.g. MetaMask). Install one and reload this page, or open it in a wallet-capable browser. Until then, do NOT manually send funds to any address.',
  '未检测到链上钱包。请在安装了链上钱包的浏览器中打开本页后重试。': 'No on-chain wallet detected. Open this page in a browser with an on-chain wallet and try again.',
  '链上配置未完成': 'On-chain configuration incomplete',
  '平台尚未配置本网络的 USDC 代币地址,无法生成存入调用。请联系平台处理 —— 在此之前请不要手动向任何地址转账。': 'The platform has not configured the USDC token address for this network, so no deposit call can be generated. Please contact us — and do NOT manually send funds to any address in the meantime.',
  '本订单的付款窗口已关闭,不能再存入。如果你已经把 USDC 发进合约,资金仍在链上合约里,请在订单页联系我们。': 'The payment window for this order is closed, so no further deposit is possible. If you already sent USDC to the contract, the funds are still held on-chain — contact us from the order page.',
  '你在钱包里取消了操作。没有发出任何交易,也没有任何扣款。': 'You cancelled in your wallet. No transaction was sent and nothing was charged.',
  '你在钱包里取消了操作。没有发出任何交易,链上状态没有任何变化。': 'You cancelled in your wallet. No transaction was sent and nothing changed on-chain.',
  '连接钱包失败': 'Could not connect the wallet',
  '读取钱包网络失败': 'Could not read the wallet network',
  '切换网络失败': 'Could not switch network',
  '钱包仍不在正确的网络上,无法继续。请在钱包里切换后重试。': 'The wallet is still on the wrong network, so this cannot continue. Switch networks in your wallet and retry.',
  '读取 USDC 余额失败': 'Could not read the USDC balance',
  '读取 USDC 授权额度失败': 'Could not read the USDC allowance',
  '钱包返回了无法解析的链上读取结果,未发出任何交易。请稍后重试。': 'The wallet returned an unreadable on-chain result. No transaction was sent — please retry later.',
  '钱包 USDC 余额不足,未发出任何交易': 'Not enough USDC in the wallet — no transaction was sent',
  '当前余额 {have} USDC,本单需要 {need} USDC,还差 {short} USDC。': 'Balance is {have} USDC, this order needs {need} USDC — {short} USDC short.',
  '余额已补足,重新检查': 'Balance topped up — check again',
  '提交授权交易失败': 'Could not submit the approval transaction',
  '授权交易未能在链上成功,未发出存入交易。请在钱包里确认后重试。': 'The approval transaction did not succeed on-chain, so no deposit was sent. Check your wallet and retry.',
  '提交存入交易失败': 'Could not submit the deposit transaction',
  '暂时无法获取存入授权,请稍后重试。未发出任何交易。': 'The deposit authorization could not be issued right now. No transaction was sent — please retry later.',
  '存入授权绑定的钱包地址与当前连接的账户不一致,已作废。请重新获取授权。': 'The deposit authorization is bound to a different wallet address than the one now connected, so it is void. Get a new authorization.',
  '存入授权已过期,已自动重新获取。': 'The deposit authorization expired and was automatically re-issued.',
  '存入授权已过期,重新获取后仍然过期。请刷新页面重试。': 'The deposit authorization expired, and the re-issued one expired too. Reload the page and retry.',
  '检测到钱包账户变更,原存入授权已作废,需要重新获取。': 'Wallet account change detected — the previous deposit authorization is void and must be re-issued.',
  '检测到钱包网络变更,原存入授权已作废,需要重新获取。': 'Wallet network change detected — the previous deposit authorization is void and must be re-issued.',
  '等待链上确认': 'Waiting for on-chain confirmation',
  '等待链上确认中…': 'Waiting for on-chain confirmation…',
  '存入交易已广播。订单状态由链上事件驱动,通常约 1–2 分钟后自动更新;这段时间可以离开本页。': 'The deposit transaction was broadcast. The order status is driven by on-chain events and usually updates itself in about 1–2 minutes — you can leave this page in the meantime.',
  '交易已广播。订单状态由链上事件驱动,通常约 1–2 分钟后自动更新;这段时间可以离开本页。': 'The transaction was broadcast. The order status is driven by on-chain events and usually updates itself in about 1–2 minutes — you can leave this page in the meantime.',
  '交易哈希': 'Transaction hash',
  '在区块浏览器查看': 'View on the block explorer',
  '同一订单在合约里只能存入一次。请不要重复提交 —— 重复的存入交易会在链上失败并白白消耗 gas。': 'An order can only be funded once in the contract. Do not submit again — a duplicate deposit reverts on-chain and wastes gas.',
  '链上已确认存入': 'Deposit confirmed on-chain',
  '担保合约已收到你的 USDC。订单状态已按链上事件更新。': 'The escrow contract received your USDC. The order status was updated from the on-chain event.',
  '链上确认时间超出预期。资金安全不受影响(以链上为准),你可以稍后刷新本页查看,或用上面的交易哈希在区块浏览器核对。': 'On-chain confirmation is taking longer than expected. This does not affect the safety of the funds (the chain is the source of truth) — reload this page later, or check the transaction hash above on the block explorer.',
  '暂时无法读取链上担保状态': 'Could not read the on-chain escrow status right now',
  ...
  '确认收货并放款(链上)': 'Confirm receipt and release on-chain',
  '包裹送达后,可在此用你自己的链上钱包确认收货并放款。本轨的确认收货不经 app 内动作完成,只由链上合约释放。': 'Once the parcel is delivered you can confirm receipt and release here, from your own on-chain wallet. On this rail confirming receipt is not an in-app action — only the on-chain contract releases the funds.',
  '有问题,冻结自动放款(链上争议)': 'Something is wrong — freeze the auto-release (on-chain dispute)',
  '提示:发起链上争议只做一件事 —— 把合约冻结、停掉自动放款,资金原样留在合约里。链上裁决(退款 / 放款 / 分账)由平台仲裁 key 执行,该能力仍在接线中,现在按下不会产生任何裁决结果。冻结后请在订单页联系我们人工跟进。': 'Note: opening an on-chain dispute does exactly one thing — it freezes the contract and stops the auto-release; the funds stay in the contract untouched. An on-chain ruling (refund / release / split) is executed by the platform arbiter key, and that capability is still being wired up — pressing this today produces NO ruling. After freezing, contact us from the order page for manual follow-up.',
  '已过可发起链上争议的时间(合约以自动放款时刻为界,不含该时刻)。': 'The window for opening an on-chain dispute has passed (the contract bounds it by the auto-release moment, exclusive).',
  '提交冻结交易失败': 'Could not submit the freeze transaction',
  '提交放款交易失败': 'Could not submit the release transaction',
  '等待链上确认放款': 'Waiting for on-chain confirmation of the release',
  '等待链上确认冻结': 'Waiting for on-chain confirmation of the freeze',
  '链上已放款': 'Released on-chain',
