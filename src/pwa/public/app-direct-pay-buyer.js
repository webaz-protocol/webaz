...
window.dpLoadBuyerAccounts = async (productId) => {
  ...
  const priceUsdc = Number(document.getElementById('dp-rail-block')?.getAttribute('data-amt') || '')
  box.innerHTML = `
    ...
    ${opts.map((o, i) => {
      ...
      const fx = (o.currency && Number.isFinite(priceUsdc)) ? window.dpFxInCurrency(priceUsdc, o.currency) : ''
      return `<label style="display:flex;gap:8px;align-items:flex-start;font-size:13px;padding:6px 2px;cursor:pointer">
        ...
        <span><b>${title}</b>${sub ? ` <span style="color:#6b7280;font-size:11px">${sub}</span>` : ''}${fx ? `<br><span style="color:#6b7280;font-size:11px">${t('卖家按此收款')} ≈ ${fx}</span>` : ''}</span>
      </label>`
    }).join('')}
