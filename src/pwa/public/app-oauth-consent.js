...
;(function () {
  ...
  const SCOPE_DESC = {
    ...
    'order:draft': () => t('获取报价(用你的默认收货地区计算;不下单/不付款/不动资金)、起草订单,并向你的人工审批队列提交接单/发货【请求】(仅提交 —— 绝不执行,执行永远需要你的 Passkey)'), 'chat:context': () => t('在你的【订单】里代你发消息给交易对方(仅限订单双方;无法私信任何其他人;反诈检测与频率限制照常生效,消息会标注为 agent 代发)'),
