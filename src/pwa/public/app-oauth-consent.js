...
;(function () {
  ...
  const SCOPE_DESC = {
    'read': () => t('读取:公开商品/搜索/你的公开资料,以及你自己的商品目录和最小化订单信息(不含买家地址/联系方式等 PII)'),
    'order:draft': () => t('获取报价(用你的默认收货地区计算;不下单/不付款/不动资金)、起草订单,并向你的人工审批队列提交接单/发货【请求】(仅提交 —— 绝不执行,执行永远需要你的 Passkey)'), 'aftersales:request': () => t('代你提交订单售后【请求】:确认收货(按订单冻结规则结算分账)、直付未付款取消、申请退货 —— 每一项都只是提交,你在 PWA 看到精确后果并用 Passkey 批准后才会执行'), 'address': () => t('查看你默认收货地址的【脱敏状态】(仅国家/地区码与是否已设置 —— 永远看不到完整地址),并可提交【地址变更请求】(你在 PWA 看到完整新地址并用 Passkey 确认后才会生效)'), 'chat:context': () => t('读取并代你发送你【订单】里与交易对方的聊天(仅限订单双方 —— 可读取该订单已有聊天记录原文;无法私信任何其他人;反诈检测与频率限制照常生效,代发消息会标注为 agent 代发)'),
