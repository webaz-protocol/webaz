...
;(function () {
  ...
  const SCOPE_DESC = {
    ...
    'order:draft': () => t('获取报价(用你的默认收货地区计算;不下单/不付款/不动资金)、起草订单,并向你的人工审批队列提交接单/发货【请求】(仅提交 —— 绝不执行,执行永远需要你的 Passkey)'), 'address': () => t('查看你默认收货地址的【脱敏状态】(仅国家/地区码与是否已设置 —— 永远看不到完整地址),并可提交【地址变更请求】(你在 PWA 看到完整新地址并用 Passkey 确认后才会生效)'), 'chat:context': () => t('读取并代你发送你【订单】里与交易对方的聊天(仅限订单双方 —— 可读取该订单已有聊天记录原文;无法私信任何其他人;反诈检测与频率限制照常生效,代发消息会标注为 agent 代发)'),
