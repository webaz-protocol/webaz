...
//   托管(escrow)=WAZ 模拟托管收入;链上担保(usdc_escrow)=真实 USDC 存入 Base 链合约(平台不能转去任意地址,合约只认买家/卖家/平台费);直接收款(direct_p2p)=场外收款。
...
;(function () {
  window.gmvRailSplitHtml = (escrow, directPay, usdcEscrow, fmt, other) => {
    ...
    return `<div style="font-size:10px;color:#6b7280;margin-top:3px;line-height:1.5" title="${t('托管=平台托管收入;链上担保=USDC 存入链上合约,平台不能转给任意地址(合约只认买家/卖家/平台费);直接收款=场外收款,平台不经手')}">🏦 ${t('托管')} ${f(e)}${u > 0 ? ` · ⛓️ ${t('链上担保')} ${f(u)}` : ''}${d > 0 ? ` · 🤝 ${t('直接收款')} ${f(d)}` : ''}${o > 0 ? ` · ❔ ${t('其他支付轨')} ${f(o)}` : ''}</div>`
  }
})()
