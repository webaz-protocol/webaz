...
window.orderErrorLookup = (code) => ({
  ...
  WITHDRAW_FAILED: t('撤回失败,请重试'), CANCEL_FAILED: t('取消失败,请重试'), DIRECT_PAY_SETTLE_FAILED: t('直付完成结算失败,订单未完成,可重试'), UNDELIVERABLE_DISABLED: t('未派送成功收口功能未启用'), UNDELIVERABLE_MARK_FAILED: t('登记未派送成功失败,请重试'), NOT_RETURN_PENDING: t('订单不在待退货确认阶段'), RETURN_EVIDENCE_REQUIRED: t('须附退货物流凭证'), RETURN_CONFIRM_FAILED: t('退货确认结算失败,请重试'),
