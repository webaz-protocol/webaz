...
window.orderErrorLookup = (code) => ({
  ...
  RAIL_DISABLED: t('WAZ 模拟托管轨已下架,请选择直付方式下单'), USDC_ESCROW_CONFIRM_NOT_WIRED: t('USDC 担保订单请在订单页用你的链上钱包完成确认收货并放款,无需此操作'), ORDER_NOT_FOUND: t('订单不存在'),
  /* USDC 合约担保(B3)建单/协商码 */ USDC_ESCROW_SELLER_NOT_READY: t('该卖家暂不支持 USDC 担保'), USDC_ESCROW_CAP_EXCEEDED: t('超出 USDC 担保单笔上限'), USDC_ESCROW_NOT_CONFIGURED: t('USDC 担保暂未配置'), USDC_ESCROW_TOO_MANY_OPEN: t('你在该卖家的进行中 USDC 担保订单已达上限'), USDC_ESCROW_SIMPLE_PRODUCT_ONLY: t('USDC 担保暂仅支持无规格商品'), USDC_ESCROW_UNSUPPORTED_OPTION: t('USDC 担保暂不支持所选订单选项'), USDC_ESCROW_CREATE_FAILED: t('USDC 担保订单创建失败,请重试'), USDC_ESCROW_MUTUAL_CANCEL_NOT_WIRED: t('USDC 担保订单的协商取消需链上退款(接线中)'), /* B6b-2 A6 退货接受入口 fail-closed(returns.ts) */ USDC_ESCROW_RETURN_NOT_WIRED: t('USDC 担保订单的退货退款需链上退款(接线中),请在订单页与对方协商或发起争议'), /* B6a voucher 签发 */ USDC_ESCROW_VOUCHER_WRONG_RAIL: t('该操作仅适用于 USDC 合约担保订单'), USDC_ESCROW_VOUCHER_NOT_OPEN: t('订单不在可签发存款授权的付款窗口'), USDC_ESCROW_VOUCHER_BAD_ADDRESS: t('无效的买家链上地址'), USDC_ESCROW_VOUCHER_ALREADY_FUNDED: t('该订单已完成链上存入,凭证不可重签'), USDC_ESCROW_VOUCHER_VOIDED: t('该订单凭证已作废(订单已取消)'), USDC_ESCROW_VOUCHER_SIGN_FAILED: t('凭证签名失败,请重试'), USDC_ESCROW_VOUCHER_RATE_LIMITED: t('签发过于频繁,请稍后再试'), USDC_ESCROW_VOUCHER_BAD_AMOUNT: t('订单金额无效'), /* B7a admin arbiter 裁决 */ USDC_ESCROW_ARBITER_WRONG_RAIL: t('该操作仅适用于 USDC 合约担保订单'), USDC_ESCROW_ARBITER_ALREADY_FINAL: t('订单已终结,不可再裁决'), USDC_ESCROW_ARBITER_NOT_DISPUTED: t('订单未处于争议态,无法链上裁决;请先在订单页开启争议'), USDC_ESCROW_ARBITER_NO_INTENT: t('该订单无 USDC 担保凭证,无法裁决'), USDC_ESCROW_ARBITER_BAD_REFUND: t('退款金额无效(须为不超过托管额的非负整数)'), USDC_ESCROW_ARBITER_RESOLVE_FAILED: t('链上裁决失败,请稍后重试或核对链上状态'), USDC_ESCROW_ARBITER_FLAG_FAILED: t('链上冻结失败,请稍后重试或核对链上状态'),
