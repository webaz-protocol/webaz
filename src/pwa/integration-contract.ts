...
export function buildIntegrationContract() {
  return {
    ...
    agent_quickstart: {
      ...
      ...(process.env.WEBAZ_REMOTE_MCP === '1' ? { remote_mcp: { url: `${BASE}/mcp`, transport: 'streamable-http (stateless, POST only)', auth: 'anonymous = public reads; Authorization: Bearer <api_key> = authenticated; RISK actions return approve_url (Passkey)' } } : {}),
