...
export const CONTRACT_CHANGES: ContractChange[] = [
  ...
  { contract_version: 9, date: '2026-07-03', surface: 'entity', kind: 'added', summary: '§① order lifecycle gains a Direct Pay Rail 1 (non-custodial) negotiation state: payment_query. When a seller reports non-receipt of an off-protocol payment, the order moves accepted→payment_query = bilateral negotiation (争议, NOT arbitration): buyer may cancel (concede) or assert paid, seller may confirm-received (payment_query→accepted, resume) or (after a buyer-silence grace) request cancel with a 7-day buyer recourse window. Only if negotiation fails (buyer asserts paid AND seller denies) does either party escalate payment_query→disputed = evidence + arbitration (reputation-only, non-custodial); and before a ruling either party may WITHDRAW the arbitration (disputed→payment_query) back to negotiation. Additive — escrow orders never enter payment_query; existing states/transitions unchanged; agents may ignore unless they handle direct_p2p disputes.', migration: 'direct_p2p sellers no longer jump straight to arbitration for non-receipt — the new accepted→payment_query negotiation stage precedes disputed. Agents reading order.status must treat payment_query as a non-terminal Direct-Pay negotiation state.' },
]
