...
export const CONTRACT_CHANGES: ContractChange[] = [
  ...
  { contract_version: 7, date: '2026-06-27', surface: 'capability', kind: 'added', summary: '§② capability matrix gains the "direct_pay" action: any write under /api/direct-pay/* (Direct Pay Rail 1) maps to it. This is a RISK scope — the handler additionally enforces a live human Passkey ceremony (agents are hard-rejected, no scope-declaration or "*" bypass). Additive — no existing action changed.', migration: 'Direct Pay write paths require a Passkey-bound human; a declared/"*" agent cannot perform them. No change to existing actions or to escrow order flows.' },
]
