...
export const CONTRACT_CHANGES: ContractChange[] = [
  ...
  { contract_version: 9, date: '2026-07-03', surface: 'entity', kind: 'added', summary: '§① order lifecycle gains a Direct Pay Rail 1 (non-custodial) negotiation state payment_query + its transitions (accepted→payment_query, payment_query→{accepted,cancelled,disputed}, disputed→payment_query withdraw). SCAFFOLD ONLY (PR-A, backend state-machine): the state + transitions exist in the machine but NO user path enters payment_query yet — the current direct_p2p "未收到货款" UI still uses the LEGACY accepted→disputed path, and PR-A adds no route rail-guard. Design intent (to be WIRED in PR-B routes + PR-C UI): seller non-receipt → payment_query = bilateral negotiation (争议) that precedes 举证仲裁 (payment_query→disputed, evidence-required), with confirm-received resume (payment_query→accepted), buyer/seller cancel + a 7-day buyer recourse window, and withdraw-before-ruling (disputed→payment_query). Additive — existing states/transitions unchanged; escrow never uses these transitions (the guard enforcing direct_p2p-only entry lands in PR-B; PR-A exposes no entry point). Agents may ignore until PR-B/PR-C land.', migration: 'No behavioural change yet — do NOT assume direct_p2p non-receipt uses payment_query at this revision; it still goes accepted→disputed (legacy). payment_query becomes reachable only when PR-B (route actions + rail guard + timing windows) and PR-C (UI) ship.' },
]
