...
export async function approveAndExecuteOrderSubmit(db: Database.Database, deps: {
  ...
  if (claim.changes !== 1 && String(reqRow.status) !== 'approved') return fail('SUBMIT_REQUEST_NOT_PENDING', 409, '提交请求已过期或已处理')
  ...
  if (!draft) return fail('DRAFT_NOT_FOUND', 404, '草稿不存在')
  ...
  if (String(draft.status) !== 'draft') return fail('DRAFT_NOT_AVAILABLE', 409, `草稿状态为 ${String(draft.status)},不可执行`)
  if (String(draft.expires_at) <= nowIso) return fail('DRAFT_NOT_AVAILABLE', 409, '草稿已过期,请重新报价')

  // ── ① params_hash 重算必须与 Passkey 绑定的一致(直接改库也骗不过) ──
  if (orderSubmitParamsHash(draft) !== String(reqRow.params_hash)) return fail('DRAFT_DRIFT', 409, '草稿内容与你 Passkey 批准的内容不一致,已拒绝执行')
  ...
  if (!addrText) return fail('ADDRESS_CHANGED', 409, '默认地址已被清空 —— 请在 webaz.xyz 重设默认地址后重新报价')
  if (draft.address_summary_hash && sha(addrText) !== String(draft.address_summary_hash)) {
    return fail('ADDRESS_CHANGED', 409, '默认地址在报价后被修改 —— 请重新报价以按新地址计算运费/可售性')
  }
  ...
  if (!created) {
    ...
    return fail(String(lb.json?.error_code || 'ORDER_CREATE_REJECTED'), 409, `建单被拒绝(${upstream})—— 草稿已退回,可修正后重新报价`)
  }
