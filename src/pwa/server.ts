...
import { initMutualCancelSchema } from '../layer3-trust/L3-1-dispute-engine/mutual-cancel.js'; import { registerMutualCancelRoutes } from './routes/mutual-cancel.js'; import { initDirectPayCancelRefundSchema } from '../direct-pay-cancel-refund.js'; import { registerDirectPayCancelRefundRoutes } from './routes/direct-pay-cancel-refund.js'; import { initDirectPayFaultRefundSchema } from '../direct-pay-fault-refund.js'; import { registerDirectFaultRefundRoutes } from './routes/direct-fault-refund.js'; import { registerDirectPayReturnsRoutes } from './routes/direct-pay-returns.js'; import { registerDirectPayPendingAcceptRoutes } from './routes/direct-pay-pending-accept.js'; import { registerShippingTemplateRoutes } from './routes/shipping-templates.js'; import { registerBondSellerRoutes } from './routes/bond-seller.js'  // 协商取消(disputed) + 直付取消退款握手(accepted):域 schema + 端点
import {
...
initSystemUser(db)
initDisputeSchema(db); initMutualCancelSchema(db); initDirectPayCancelRefundSchema(db); initDirectPayFaultRefundSchema(db)   // + mutual_cancel_proposals / direct_pay_cancel_requests / direct_fault_refund_requests 表
...
initProductVariantsSchema(db); initRecommendationAnchorSchema(db)
...
}); registerUsdcPayoutAddressRoutes(app, { db, auth, isTrustedRole, generateId }); registerUsdcEscrowRoutes(app, { db, auth, isTrustedRole, getProtocolParam, escrowVoucherAccount: () => walletSigner.escrowVoucherAccount(), rateLimitOk }); registerUsdcEscrowArbiterRoutes(app, { db, requireProtocolAdmin: (req, res) => requireAdminPermission(req, res, 'protocol'), consumeGateToken, logAdminAction, network: (process.env.NETWORK || 'testnet').toLowerCase() === 'mainnet' ? 'mainnet' : 'testnet', resolveDisputeOnChain: (a) => resolveDisputeOnChain({ arbiterWalletClient: createWalletClient({ account: walletSigner.arbiterAccount(), chain: ACTIVE_CHAIN, transport: http(rpcUrl) }) as unknown as ArbiterWalletClient, publicClient: publicClient as unknown as ArbiterPublicClient, contractAddress: process.env.USDC_ESCROW_CONTRACT }, a), flagDisputeOnChain: (a) => flagDisputeOnChain({ arbiterWalletClient: createWalletClient({ account: walletSigner.arbiterAccount(), chain: ACTIVE_CHAIN, transport: http(rpcUrl) }) as unknown as ArbiterWalletClient, publicClient: publicClient as unknown as ArbiterPublicClient, contractAddress: process.env.USDC_ESCROW_CONTRACT }, a) })   // USDC 合约担保 PR-B2 收款地址 + B6a voucher 签发/状态(限流 10/min/user)+ B7a arbiter Passkey 门裁决(arbiterResolve 移动真钱,现场真人 Passkey 绑动作+order_id)
...
function settleCommission(orderId: string, effectiveBase?: number): CommissionResult {
  ...
  const _liveRdRow = db.prepare("SELECT return_days FROM products WHERE id = ?").get(order.product_id) as { return_days: number | null } | undefined; const returnDaysClearing = _liveRdRow === undefined ? 7 : effectiveReturnDays((order as unknown as Record<string, unknown>).trade_terms_snapshot, _liveRdRow.return_days ?? 7).days   // RFC-026:清算窗与买家退货窗同源冻结(terms-as-sold),缺商品行保留旧默认 7
...
registerWebauthnRoutes(app, {
  ...
  getProtocolParam, createOrderLoopback: makeCreateOrderLoopback(() => PORT), apiLoopback: makeApiLoopback(() => PORT),   // RFC-025 PR-3 quote 参数 + PR-5a 回环建单 + RFC-026 PR-4 聊天回环(真实路径单一真相源;见 order-loopback.ts)
})
// RFC-028 S1a: explicit PWA boot registration; the helper self-initializes its OAuth parent.
// Inert schema only — no gateway middleware or commerce route is mounted.
