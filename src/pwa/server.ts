...
import { registerWebauthnRoutes } from './routes/webauthn.js'; import { makeCreateOrderLoopback } from './order-loopback.js'
...
registerWebauthnRoutes(app, {
  ...
  getProtocolParam, createOrderLoopback: makeCreateOrderLoopback(() => PORT),   // RFC-025 PR-3 quote 参数 + PR-5a 回环建单(真实 /api/orders 单一执行真相源;见 order-loopback.ts)
})
