...
export function executeSellerOrderAction(db: Database.Database, opts: ExecOpts): ExecResult {
  ...
  const auditTag = (code: string) => `order_action:execute:${orderId}:${action}:path=${path}:strict=${opts.strictTracking}${code ? ':' + code : ''}`
  const fail = (error_code: string, http: number, error?: string): ExecResult => { opts.audit?.(auditTag(error_code), 'deny'); return { ok: false, error_code, http, error } }

  const order = db.prepare('SELECT seller_id, status, accept_deadline, ship_deadline FROM orders WHERE id = ?').get(orderId) as { seller_id: string; status: string; accept_deadline: string | null; ship_deadline: string | null } | undefined
  ...
  // 核心守卫②③:状态前置 + SLA(deadline 未过)
  if (action === 'accept') {
    if (order.status !== 'paid') return fail('WRONG_STATUS', 409, '仅 paid 订单可接单')
    if (order.accept_deadline && nowIso > order.accept_deadline) return fail('SLA_EXPIRED', 409, '接单窗口已过')
  } else {
    if (order.status !== 'accepted') return fail('WRONG_STATUS', 409, '仅 accepted 订单可发货')
    if (order.ship_deadline && nowIso > order.ship_deadline) return fail('SLA_EXPIRED', 409, '发货窗口已过')
    // I4(仅 agent-approve 路径 strictTracking=true):tracking 内容重校,不信任 request 内容
    if (opts.strictTracking) {
  ...
  if (action === 'ship') {
    const desc = opts.strictTracking
 ...
 *   执行失败(守卫/transition):【不】写 executed_at,请求保持 approved 可重试;写失败 execution_result(注解)+ audit。
...
export function approveAndExecuteOrderAction(db: Database.Database, requestId: string, actorId: string, grantId: string, nowIso: string, generateId: (p: string) => string): { ok: boolean; order_status?: string; already_executed?: boolean; error?: string; error_code?: string; http?: number } {
  // 审计内置(domain 层,不增 route seam):写 agent_grant_auth_log
  const audit = (capability: string, outcome: 'allow' | 'deny') => { try { db.prepare('INSERT INTO agent_grant_auth_log (grant_id, human_id, capability, outcome, error_code) VALUES (?,?,?,?,?)').run(grantId, actorId, capability, outcome, null) } catch { /* 审计写失败不阻断已判定结果 */ } }
  ...
  // step 1:CAS→approved(仅 pending;单独提交 → 执行失败仍保持 approved 可重试)。expires_at 原子守卫。
  if (r.status === 'pending') {
    ...
    if (c !== 1) return { ok: false, error_code: 'REQUEST_NOT_PENDING_OR_EXPIRED', http: 409 }
    audit(`order_action:approve:${r.order_id}:${r.order_action}`, 'allow')
  }
  // step 2:执行 + 标记 executed,原子(并发/重放回滚)
  ...
  try {
    const out = db.transaction((): ExecResult => {
      ...
      const exec = executeSellerOrderAction(db, { orderId: r.order_id, action: r.order_action as OrderExecAction, actorId, nowIso, strictTracking: true, tracking: params.tracking as string | undefined, evidenceRef: params.evidence_ref as string | undefined, generateId, path: 'approve', audit })
    ...
    if (err instanceof ExecFailErr) {
      try { db.prepare("UPDATE agent_permission_requests SET execution_result=? WHERE id=? AND executed_at IS NULL").run(JSON.stringify({ ok: false, error_code: err.exec.error_code }), requestId) } catch { /* 注解写失败不影响 */ }
      return { ok: false, error_code: err.exec.error_code, error: err.exec.error, http: err.exec.http }   // 请求保持 approved 可重试
    }
    return { ok: false, error_code: 'EXECUTE_FAILED', error: (err as Error).message, http: 500 }
  }
}
