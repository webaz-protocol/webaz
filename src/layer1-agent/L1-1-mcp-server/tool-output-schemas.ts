...
const statusObject = { type: 'object', description: 'v2 status {code,label,label_en}; code canonical (never from label)' }
const posIntQty = { type: ['integer', 'null'], minimum: 1, description: 'positive integer, or null when invalid (see quantity_valid); display-only (amount=price.amount_minor)' }
const qtyValid = { type: 'boolean', description: 'false only when quantity is invalid data (card shows 数量数据异常, buttons disabled)' }
const qtyError = { type: 'string', description: 'invalid-quantity code: missing|empty|zero|negative|not_integer|overflow|non_numeric' }
const err = {
...
export const OUTPUT_SCHEMAS: Record<string, Record<string, unknown>> = {
  webaz_search: {
    ...
    properties: {
      ...
      next_cursor: { type: 'string', description: 'more results exist — pass back as cursor' },
      sellers: { type: 'object', description: 'deduped seller summaries keyed by seller id (products[].seller_ref)' },
      fx: { type: 'object', description: 'display-only USD-base rates ({rates,as_of}) for ≈local hints; never a settlement path' },
      products: {
        type: 'array',
        items: {
          type: 'object',
          properties: {
            ...
            decision_flags: { type: 'array', description: 'risk/advantage facts {code,severity,label}' },
  ...
  webaz_buyer_orders: {
    ...
    properties: {
      ...
      timeline: { type: 'array', description: 'full form: {from,to_status,actor,at} events' },
      refund: { type: 'object', description: 'rail-honest refund state; ABSENT when no return requests' },
      ...
      summary: { type: 'object', description: 'account counts: total/active/awaiting_you/disputed/completed/cancelled/other_terminal' },
      count: { type: 'number' }, total_count: { type: 'number' },
      next_cursor: { type: 'string', description: 'older orders exist — pass back as cursor' },
      orders: { type: 'array', description: 'minimal 7-key: order_id/status/next_actor/deadline/amount/item_ref/payment_rail' },
      order: { type: 'object', description: 'single-order form (order_id arg): same 7-key projection' },
      up_to_date: { type: 'boolean', description: 'full+updated_since: true = nothing changed' },
      incremental: { type: 'object', description: 'full+updated_since with changes: {since,timeline_new} (only newer)' },
      ...err,
    },
  },
  webaz_quote_order: {
    ...
    properties: {
      ...
      price: productMoney,
      fiat_estimate: { type: 'object', description: 'display-only ≈local-fiat {currency,display,rate,as_of,stale}; never a settlement amount; omitted when unavailable' },
      ...
      payment_rail: { type: 'string' }, rail_note: { type: 'string', description: 'honesty note: simulated escrow ≠ real custody; direct_p2p = no principal held' }, rail_note_en: { type: 'string', description: 'English rendering of rail_note (widget picks under en locale)' },
  ...
  webaz_order_draft: {
    ...
    properties: {
      ...
      destination: { type: 'object' }, payment_rail: { type: 'string' }, rail_note: { type: 'string' }, rail_note_en: { type: 'string' },
      expires_at: { type: 'string' },
  ...
  webaz_submit_order_request: {
    ...
    properties: {
      ...
      on_approval: { type: 'string', description: 'approval creates the single order; payment follows the disclosed rail (direct_p2p: no principal held)' },
      approval_url: { type: 'string' },
      duplicate: { type: 'boolean' }, duplicate_warning: { type: 'object', description: 'legacy: existing request REUSED, no second approval/order' },
      // BUG-08 machine-readable duplicate contract (old clients still read the boolean):
      duplicate_reason: { type: 'string', description: 'SAME_DRAFT_REPLAY|SAME_IDEMPOTENCY_KEY|ACTIVE_INTENT_REUSED|DATABASE_UNIQUE_RACE|RESPONSE_LOSS_RECONCILED — render the precise reason' },
      duplicate_of: { type: 'string' }, existing_request_id: { type: 'string' },
      purchase_intent_instance: { type: 'string' }, reorder: { type: 'object', description: '{product_id,quantity} for 再买一份 re-quote' },
      available_actions: { type: 'array' }, disclosures: { type: 'array' },
      ...err,
    },
  },
