...
export const TOOL_ANNOTATIONS: Record<string, McpToolAnnotations> = {
  ...
  webaz_ui_spike:            { readOnlyHint: true,  destructiveHint: false, openWorldHint: true },  // EXPERIMENTAL: MCP Apps 渲染探测 —— 读公开市场对象(与 search 同 openWorld 定性),零 auth 零写 // read: own wallet balances/refund landings (RFC-026 PR-3); OAuth wallet surface is read-only forever (grant-path audit exempt)
