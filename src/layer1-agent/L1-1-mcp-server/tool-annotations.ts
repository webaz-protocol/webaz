...
export const TOOL_ANNOTATIONS: Record<string, McpToolAnnotations> = {
  ...
  webaz_wallet_view:         { readOnlyHint: true,  destructiveHint: false, openWorldHint: false },
  webaz_ui_spike:            { readOnlyHint: true,  destructiveHint: false, openWorldHint: false }, // EXPERIMENTAL: MCP Apps 渲染探测(公开数据 3 件,零 auth 零写) // read: own wallet balances/refund landings (RFC-026 PR-3); OAuth wallet surface is read-only forever (grant-path audit exempt)
