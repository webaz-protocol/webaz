...
export function createFeePrepayRequest(
  ...
  const amount = Math.floor(Number(input.amountUnits))
  if (!Number.isFinite(amount) || amount <= 0) return { ok: false, reason: 'amount must be a positive integer (base units)' }
  ...
  const pid = input.platformAccountId == null || !String(input.platformAccountId).trim() ? null : String(input.platformAccountId).trim()
  if (pid) {
