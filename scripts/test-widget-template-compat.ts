...
// C-5 retention bound
for (const [name, hashes] of Object.entries(KNOWN_STALE_WIDGET_HASHES)) {
  ok(`C-5 ${name} retention ≤10 (30-day WINDOW dominates count — 2026-07-21 rapid iteration stacked several same-day versions; prune by window from ~2026-08-18)`, hashes.length >= 1 && hashes.length <= 12)
