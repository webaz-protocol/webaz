...
ok('F3 quote card uses etaDisplay fallback behind display_eta (i18n)', /out\.display_eta\|\|etaDisplay\(s\.estimated_days/.test(QUOTE_APPROVAL_WIDGET_HTML))
...
ok('A2.2 error body surfaced, never swallowed as unknown (i18n)', /状态:查询失败\([^']*'[^)]*\)\+String\(d\.error_code\|\|d\.error\)/.test(QUOTE_APPROVAL_WIDGET_HTML))
...
ok('A2 QuoteApproval prefers display_expires_at (i18n via __i18nExp)', /__i18nExp\(out\.display_expires_at,out\.expires_at\)/.test(QUOTE_APPROVAL_WIDGET_HTML))
