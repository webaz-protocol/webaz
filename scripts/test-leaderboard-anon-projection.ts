...
try {
  ...
  // usr_id dropped from EVERY user board — all cards nav via the public handle (#shop/:identifier
  // and #u/:user_id both resolve a handle). creators previously kept id (deferred); now removed.
  for (const kind of ['buyers', 'sellers', 'agents', 'arbitrators', 'verifiers', 'creators']) {
    ok(`${kind}: canonical user id dropped (no enumeration seed)`, !BOARD_ALLOWLIST[kind].includes('id'))
  }
  // …and every user board that renders a nav link keeps `handle` (the public nav key that replaces id)
  for (const kind of ['buyers', 'sellers', 'creators', 'agents', 'arbitrators', 'verifiers']) {
    ok(`${kind}: keeps handle (public nav key)`, BOARD_ALLOWLIST[kind].includes('handle'))
  }
