...
const REMAINING_SYNC_PREPARES: Record<string, number> = {
  ...
  'direct-pay-timeouts.ts': 10,   // Direct Pay (Rail 1) 超时 cron:扫描读(付款窗口/宽限/货款协商×2/接单窗 5 扫)+ 状态转移/释放质押的 db.transaction 写仍同步(money/state path,Phase 3 迁 pg);v16 +1:pending_accept 接单窗超时扫描(sweep E,无责取消+回补,零资金)
  'direct-pay-pending-accept.ts': 1,  // 手动接单(v16):accept 的 db.transaction 内设付款窗 deadline(与状态转移同原子);其余读写全走 async seam
