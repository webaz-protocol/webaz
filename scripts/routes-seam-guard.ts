...
const REMAINING_SYNC_PREPARES: Record<string, number> = {
  ...
  'orders-action.ts': 19,  // state-machine/decline/settle 写序列 + confirm-in-person tx + 逐单 batch-ship 写(纯校验读已迁 seam)
  ...
  'returns.ts': 10,        // executeReturnRefund 退款 db.transaction(CAS return 行 + 余额守卫扣款 + 库存 + 状态,8)+ escalate 建争议 tx(2);端点校验读/列表/状态写/消息/通知已迁 seam。Codex #235 P1:tx 内先用 caller 传入的 fromStatus(pending/picked_up)CAS 抢占 return 行再扣款防并发双退,删去冗余卖家余额预读(11→10)
