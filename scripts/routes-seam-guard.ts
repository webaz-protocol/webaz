...
const REMAINING_SYNC_PREPARES: Record<string, number> = {
  ...
  'orders-action.ts': 22,  // state-machine/decline/settle 写序列 + confirm-in-person tx + 逐单 batch-ship 写 + pq_withdraw/dispute_withdraw_confirm 原子(dispute dismiss + transition + settle 同一 tx)+ PR-B mark_undeliverable 原子块(transition→delivery_failed + 置 delivery_failed_deadline 同一 db.transaction,必须 db.prepare 不可走 async seam)(纯校验读已迁 seam)
