...
async function main() {
  ...
  // ── 1. happy path: approve mints grant + code ──
  {
    ...
    ok('1f. capabilities = SAFE mapping of read+order:draft', JSON.stringify(capNames) === JSON.stringify(['buyer_case_prepare', 'buyer_discover', 'buyer_orders_read_minimal', 'draft_order', 'order_action_request', 'order_submit_request', 'price_quote', 'profile_read', 'read_public', 'search', 'seller_orders_read_minimal', 'seller_products_read']))
