...
put.run({ id: 'a-dismissed', product_id: 'p-a', seller_id: 'seller-a', buyer_id: 'buyer-d', category_tag: 'quality', winner: 'dismissed', resolution: 'withdrawn' })
...
async function main(): Promise<void> {
  ...
  ok('seller filter returns only the requested seller rows', r.status === 200 && items.length === 4 && items.every((item: any) => item.id.startsWith('a-')), JSON.stringify(items))
  ok('summary preserves every supported outcome separately', r.json.summary?.total === 4 && r.json.summary?.seller_wins === 1 && r.json.summary?.seller_losses === 1 && r.json.summary?.split === 1 && r.json.summary?.dismissed === 1, JSON.stringify(r.json.summary))
  ok('category counts use the same seller filter', (r.json.category_counts || []).reduce((sum: number, row: any) => sum + Number(row.n || 0), 0) === 4, JSON.stringify(r.json.category_counts))
  const sellerCategory = await get('/api/disputes/cases?seller_id=seller-a&category=quality&limit=50')
  ok('seller category counts remain seller-scoped without hiding its other filter choices', (sellerCategory.json.category_counts || []).reduce((sum: number, row: any) => sum + Number(row.n || 0), 0) === 4, JSON.stringify(sellerCategory.json.category_counts))
  const publicCategory = await get('/api/disputes/cases?category=quality&limit=50')
  ok('global category counts retain the prior all-category filter behavior', (publicCategory.json.category_counts || []).reduce((sum: number, row: any) => sum + Number(row.n || 0), 0) === 5, JSON.stringify(publicCategory.json.category_counts))
  ...
  const openapi = JSON.parse(readFileSync('src/pwa/public/openapi.json', 'utf8'))
  ...
  for (const key of ['公开裁决', '部分责任', '裁决已撤销', '仅展示已公开、已脱敏的终局裁决']) ok(`i18n EN exists: ${key}`, new RegExp(`'${key}'\\s*:`).test(i18n))
  ok('dismissed rulings stay neutral and retain a public-seller destination', /winner === 'dismissed'/.test(rulings) && /seller-ruling-neutral-chip/.test(rulings))
  ok('OpenAPI exposes the seller filter without hand-editing generated output', (openapi.paths?.['/api/disputes/cases']?.get?.parameters || []).some((p: any) => p.name === 'seller_id' && p.in === 'query'))
  if (fail) { console.error(`\n❌ public seller rulings failed\n${failures.join('\n')}`); process.exit(1) }
  console.log(`\n✅ public seller rulings: seller-filtered, redacted final cases + accurate outcome summary\n  ✅ pass ${pass}`)
}
