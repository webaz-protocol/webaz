...
db.prepare("INSERT INTO return_requests (id,order_id,buyer_id,seller_id,product_id,reason,refund_amount,status,resolved_at) VALUES ('rr1','ord_f','buyer1','seller1','prd_f','quality',7.06,'refunded',?)").run(FUTURE)
...
try {
  ...
  ok('F-8 available_actions server-authoritative: delivered → human confirm/dispute/return + agent prepare_case,每项带 executor',
    acts.some(a => a.action === 'confirm_receipt' && a.executor === 'human_order_page')
    && acts.some(a => a.action === 'open_dispute' && a.executor === 'human_order_page')
    && acts.some(a => a.action === 'request_return' && a.executor === 'human_order_page')
