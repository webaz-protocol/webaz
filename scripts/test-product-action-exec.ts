...
try {
  // 1. approved request → execute via approval
  {
    ...
    const res = executeProductActionRequest(db, { requestId: r })
  ...
  {
    const p = mkProduct(alice), r = mkReq(alice, p, 'approved')
    executeProductActionRequest(db, { requestId: r })
    const res2 = executeProductActionRequest(db, { requestId: r })
  ...
  // 8. PRODUCT_NOT_FOUND / REQUEST_NOT_FOUND / REVOKED
  {
    ...
    ok('8a product gone → PRODUCT_NOT_FOUND 404', executeProductActionRequest(db, { requestId: rGhost }).error_code === 'PRODUCT_NOT_FOUND')
    ok('8b unknown request → REQUEST_NOT_FOUND 404', executeProductActionRequest(db, { requestId: 'par_ghost' }).error_code === 'REQUEST_NOT_FOUND')
    const p = mkProduct(alice), rRev = mkReq(alice, p, 'revoked')
    ok('8c revoked request → REQUEST_REVOKED 409, product intact', executeProductActionRequest(db, { requestId: rRev }).error_code === 'REQUEST_REVOKED' && exists(p))
  }
  ...
  {
    const p = mkProduct(alice), r = mkReq(alice, p, 'approved')
    const res = executeProductActionRequest(db, { requestId: r, retireAnchorsByTarget: () => { throw new Error('anchor boom') } })
  ...
  //   delete that doesn't happen. Forced via a proxy that makes the claim UPDATE report 0 changes.
  {
    ...
    const res = executeProductActionRequest(proxy as never, { requestId: r })
  ...
  // 13. sanitized failure: a db missing the requests table → EXEC_FAILED, no raw SQL text
  {
    ...
    const res = executeProductActionRequest(bare, { requestId: 'par_x' })
