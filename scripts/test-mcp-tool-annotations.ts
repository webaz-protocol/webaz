...
async function main(): Promise<void> {
  ...
  ok('3. current tool count == 55 (derived from live tools + map; +webaz_buyer_action_request + webaz_upload_product_image)', tools.length === mapKeys.length && tools.length === 55)
  ...
  ok('4q3. webaz_buyer_action_request additive after-sales approval request (F,F,T)', eq('webaz_buyer_action_request', false, false, true))
