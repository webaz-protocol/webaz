...
// ── 43. 首单 UX 优化:文案准确化 + 融合付款弹窗(可复制三要素)+ 重复注册提醒修复。 ──
{
  ...
  ok('43b. fused modal copy-rows for account + amount + reference (dpCopyBtn per row)', /window\.dpCopyBtn\(copyText\)/.test(PM) && /row\(t\('收款账号 \/ 说明'\)[\s\S]{0,40}instr\)/.test(PM) && /row\(t\('应付金额'\)[\s\S]{0,30}amt\)/.test(PM) && /window\.dpPayRef\(oid\)/.test(PM) && /dpPayModalAmount/.test(PM))
  ...
  ok('43h. dpPromptRegisterPasskey gates registration prompt on NO_PASSKEY_REGISTERED (not blanket)', /code === 'NO_PASSKEY_REGISTERED'/.test(DP) && /if \(!notReg\)/.test(DP))
  ...
  for (const k of ['了解直接付款(需 Passkey)', '请按以下信息付款', '收款账号 / 说明', '应付金额', '付款附言(务必填写,用于卖家核对)', '稍后在订单页处理', '验证未完成,请重试', '非担保交易:WebAZ 不托管本金、无退款能力,仅对卖家有信誉处罚权']) ok(`43-i18n EN present: ${k.slice(0, 8)}`, new RegExp(`'${k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}'\\s*:`).test(I18N))
}
