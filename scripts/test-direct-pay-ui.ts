...
// ── 28. seller "未收到货款" path — LEGACY (pre-payment_query). Today the seller at accepted opens the
//    evidence dispute directly (accepted→disputed). This is the CURRENT behaviour, NOT proof that the new
//    payment_query negotiation stage is active — PR-C will rewire this button to accepted→payment_query. ──
{
  ...
  ok('28b. [LEGACY] direct-pay seller @accepted 未收到货款 still → dispute directly (rewired to payment_query in PR-C)', /payment_rail === 'direct_p2p' \?[\s\S]*action: 'dispute', label: '未收到货款'/.test(shipLine) && /needsEvidence: true/.test(shipLine))
  ok('28c. non-direct-pay (escrow) seller @accepted does NOT get the 未收到货款 dispute', /: \[\]\)\]/.test(shipLine))
  ok('28d. [LEGACY] transition accepted→disputed still allows seller (payment_query not yet wired into UI/route)', /'accepted→disputed'/.test(readFileSync('src/layer0-foundation/L0-2-state-machine/transitions.ts', 'utf8')))
  for (const k of ['未收到货款', '未收到货款说明']) ok(`28-i18n EN present: ${k}`, new RegExp(`'${k}'\\s*:`).test(I18N))
}
...
ok('29c. memo input prefilled with the reference (id=dp-buyer-memo)', /dpMemoInputHtml/.test(MEMO) && /id="dp-buyer-memo" value="\$\{escHtml\(ref\)\}"/.test(MEMO))
ok('29d. dpReadMemo reads the input, falls back to the ref, tags 付款参考', /dpReadMemo = \(orderId\) =>[\s\S]*dp-buyer-memo[\s\S]*dpPayRef\(orderId\)[\s\S]*付款参考/.test(MEMO))
...
//    memo flows notes → order_state_history → timeline; that + evidence descriptions + actor names must be escHtml'd. ──
{
  ...
  const od = APP.slice(APP.indexOf("<span class=\"detail-label\">${t('商品')}"), APP.indexOf("<span class=\"detail-label\">${t('商品')}") + 900)
  ...
  ok('30h. server bounds notes length', /const notes = String\(req\.body\?\.notes \?\? ''\)\.slice\(0, 500\)/.test(readFileSync('src/pwa/routes/orders-action.ts', 'utf8')))
}
...
//    must NOT say 退款/资金释放/仲裁费. Escrow copy unchanged. ──
{
  ...
  ok('31g. arbitrator panel uses dpArbFeeNote + dpArbRulingOptions (dispute.payment_rail)', /window\.dpArbFeeNote\(dispute\.payment_rail\)/.test(APP) && /window\.dpArbRulingOptions\(dispute\.payment_rail\)/.test(APP))
  ...
  // 33a: returns/refund cards on a completed order are suppressed for direct_p2p (backend already rejects; kill the false affordance)
  ok('33a. return cards rail-guarded (buyer + seller) for direct_p2p', (APP.match(/order\.status === 'completed'[^`]*order\.payment_rail !== 'direct_p2p'/g) || []).length >= 2)
