...
//    memo flows notes → order_state_history → timeline; that + evidence descriptions + actor names must be escHtml'd. ──
{
  ...
  // 锚在 商品 label 收尾 + value 开头(对 label/value 加 style 属性稳健;product 详情行唯一)
  const _pm = "${t('商品')}</span><span class=\"detail-value\""; const _pi = APP.indexOf(_pm); const od = _pi >= 0 ? APP.slice(_pi, _pi + 900) : ''
