...
//    must NOT say 退款/资金释放/仲裁费. Escrow copy unchanged. ──
{
  ...
  ok('34a. arbitration order-chain payload rail-aware (non-WAZ-custody rails → non_custodial + null amounts)', /const ncRail = railOutsideWazCustody\(dispute\.payment_rail\)/.test(DW) && /non_custodial: ncRail \|\| undefined/.test(DW) && /refund_amount: ncRail \? null/.test(DW) && /liability_parties: ncRail \? null/.test(DW))   // B3:谓词升级为 railOutsideWazCustody(usdc_escrow 同样绝不把托管退款语义写进签名链)
...
// ── 43. 首单 UX 优化:文案准确化 + 融合付款弹窗(可复制三要素)+ 重复注册提醒修复。 ──
{
  ...
  ok('43k. route accepts stage:both → records both stages (contract v14)', /stage !== 'both' && !VALID_STAGES\.includes/.test(readFileSync('src/pwa/routes/direct-pay-disclosure-acks.ts', 'utf8')) && /CONTRACT_VERSION = 30/.test(readFileSync('src/version.ts', 'utf8')))
...
// ── 41. 审计项 G/H(收尾):admin rail 区分 + expired 状态买家双出口。 ──
{
  ...
  // B6b-1:币种由 WAZ 白名单决定(WAZ 只属 WAZ 轨:payment_rail 缺省=历史单,或 'escrow');usdc_escrow 等其他轨=USDC。
  ok('41d. G: admin 订单列表按轨显示(直付徽标 + USDC;WAZ 轨仍 WAZ,其余轨 USDC)', /o\.payment_rail === 'direct_p2p' \? [\s\S]{0,220}\$\{t\('直付'\)\}[\s\S]{0,80}USDC` : `\$\{o\.total_amount\} \$\{\(!o\.payment_rail \|\| o\.payment_rail === 'escrow'\) \? 'WAZ' : 'USDC'\}`/.test(APP))
