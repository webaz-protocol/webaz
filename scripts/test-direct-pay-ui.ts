...
// ── 28. seller "未收到货款" path: direct-pay non-custodial → buyer's "已付款" is an unverified claim, so the seller
//    at accepted must be able to open an (evidence-based, reputation-only) dispute instead of being forced to ship. ──
{
  ...
  ok('28b. direct-pay seller @accepted ALSO gets 未收到货款 → dispute (rail-scoped)', /payment_rail === 'direct_p2p' \?[\s\S]*action: 'dispute', label: '未收到货款'/.test(shipLine) && /needsEvidence: true/.test(shipLine))
  ok('28c. non-direct-pay (escrow) seller @accepted does NOT get the 未收到货款 dispute', /: \[\]\)\]/.test(shipLine))
  ok('28d. transition accepted→disputed already allows seller (no contract change)', /'accepted→disputed'/.test(readFileSync('src/layer0-foundation/L0-2-state-machine/transitions.ts', 'utf8')))
