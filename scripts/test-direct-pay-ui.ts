...
// ── 43. 首单 UX 优化:文案准确化 + 融合付款弹窗(可复制三要素)+ 重复注册提醒修复。 ──
{
  ...
  ok('43k. route accepts stage:both → records both stages (contract v14)', /stage !== 'both' && !VALID_STAGES\.includes/.test(readFileSync('src/pwa/routes/direct-pay-disclosure-acks.ts', 'utf8')) && /CONTRACT_VERSION = 17/.test(readFileSync('src/version.ts', 'utf8')))
