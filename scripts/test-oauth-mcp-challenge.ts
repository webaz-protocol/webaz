...
const GRANT_PATH_TOOLS = ['webaz_list_product', 'webaz_get_agent_order', 'webaz_order_action_request', 'webaz_connection_status', 'webaz_buyer_orders', 'webaz_discover', 'webaz_quote_order', 'webaz_order_draft', 'webaz_submit_order_request', 'webaz_prepare_case', 'webaz_approval_requests', 'webaz_wallet_view', 'webaz_order_chat']
...
async function main() {
  ...
  // (曾经 list:draft → list_product_draft,没有任何端点消费它 → 合规客户端完成 OAuth 后永远 PERMISSION_REQUIRED)。
  {
    ...
    const TOOL_REQUIRED_CAPS: Record<string, string[]> = {
      ...
      webaz_buyer_orders: ['buyer_orders_read_minimal'],   // GET /api/agent/buyer/orders(/:id) — RFC-025 PR-1
      ...
      webaz_prepare_case: ['buyer_case_prepare'],   // GET /api/agent/buyer/orders/:id/case-draft — RFC-025 PR-6
      webaz_approval_requests: ['approval_requests_read'],   // GET /api/agent/approval-requests(/:id) — RFC-026 PR-2
