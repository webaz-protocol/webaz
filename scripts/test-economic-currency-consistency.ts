...
try {
  ...
  ok('webaz_discover max_price contract says USDC (not the old WAZ); simulated-alias honesty lives in the discover response pricing_note (tools/list budget is tight)', /Budget ceiling in USDC/.test(SERVER) && !/Budget ceiling in WAZ/.test(SERVER))
