...
}
{
  ...
  ok('U-3 webaz_search WIRE descriptor carries openai/outputTemplate → versioned ui://widget/webaz-products.*.html',
    (() => { const ot = String(searchTool?._meta?.['openai/outputTemplate'] ?? ''); return ot.startsWith('ui://widget/webaz-products.') && /\.[0-9a-f]{8,}\.html$/.test(ot) })(), JSON.stringify(searchTool?._meta ?? null))
