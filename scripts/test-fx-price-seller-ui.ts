...
ok('3c. order total → USDC (orderAmountHtml, PR-1d); only wallet-refund line keeps WAZ', APP.includes('${window.orderAmountHtml(order)}') && APP.includes("从卖家钱包退')} ${order.total_amount} WAZ"))
