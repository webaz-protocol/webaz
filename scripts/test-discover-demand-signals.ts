...
try {
  ...
  { const r = await mcp.handleDiscover({ category: 'phone_stand', max_price: 50 })   // 命中(预算滤掉 900)
    ...
    ok('D-5b detail_fetch_template = webaz_search(result_handle, selected_ids, card:true) with UP TO 5 ids (not 1) — card:true renders the STANDARD search card', (() => { const t = r.detail_fetch_template as { tool?: string; arguments?: { result_handle?: string; selected_ids?: unknown[]; card?: unknown } } | undefined; return !!t && t.tool === 'webaz_search' && t.arguments?.result_handle === r.result_handle && Array.isArray(t.arguments?.selected_ids) && (t.arguments!.selected_ids as unknown[]).length === Math.min(5, cands!.length) && t.arguments?.card === true })())
