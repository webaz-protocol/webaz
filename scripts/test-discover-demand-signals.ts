...
try {
  ...
  { const r = await mcp.handleDiscover({ category: 'phone_stand', max_price: 50 })   // 命中(预算滤掉 900)
    ...
    ok('D-5b detail_fetch_template = webaz_search(result_handle, selected_ids) with UP TO 5 ids — NO undeclared params (方案A:多选默认即标准卡;strict 宿主会剥/拒未声明参数)', (() => { const t = r.detail_fetch_template as { tool?: string; arguments?: Record<string, unknown> } | undefined; return !!t && t.tool === 'webaz_search' && t.arguments?.result_handle === r.result_handle && Array.isArray(t.arguments?.selected_ids) && (t.arguments!.selected_ids as unknown[]).length === Math.min(5, cands!.length) && !('card' in (t.arguments ?? {})) && JSON.stringify(Object.keys(t.arguments ?? {}).sort()) === JSON.stringify(['result_handle', 'selected_ids']) })())
    ok('D-5c display_hint steers a multi-product comparison card + forbids the single-title funnel', typeof r.display_hint === 'string' && /comparison card|up to 5/i.test(String(r.display_hint)) && /do not copy a candidate title/i.test(String(r.display_hint)) && !/exact product title you picked/i.test(String(r.display_hint))) }
