...
async function main(): Promise<void> {
  ...
  ok('1. all 43 remote-visible tools carry a non-empty securitySchemes array on the WIRE (webaz_pair local-only hidden)', tools.length === 43 && tools.every(t => Array.isArray(t.securitySchemes) && t.securitySchemes.length > 0))
  ...
  ok('5. EXACTLY the 5 grant-reachable tools advertise oauth2 (no false OAuth anywhere else)',
    JSON.stringify(oauthTools) === JSON.stringify(['webaz_buyer_orders', 'webaz_connection_status', 'webaz_get_agent_order', 'webaz_list_product', 'webaz_order_action_request']))
  ...
  console.log(`✅ mcp securitySchemes: 43/43 on remote wire (webaz_pair hidden) · oauth2 ONLY for the 5 grant-reachable (exact scopes) · noauth everywhere else (no false OAuth on api_key-only)\n  ✅ pass ${pass}`)
}
