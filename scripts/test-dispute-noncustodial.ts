...
const { initArbitratorReviewSchema } = await import('../src/runtime/webaz-schema-helpers.js')  // PR-B: arbitrator_whitelist 授权源
...
initArbitratorReviewSchema(db)   // PR-B: arbitrator_whitelist(engine 授权源改为 active 白名单)
...
db.prepare("INSERT INTO arbitrator_whitelist (user_id, status) VALUES ('arb1','active')").run()  // PR-B: active 授权源(role 旁路已移除)
