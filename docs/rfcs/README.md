...
| [RFC-021](RFC-021-order-action-request.md) | Order-action request — first **approve-to-execute** delegated order action (accept/ship): agent submits (SAFE scope, zero api_key) → human Passkey approves (binds order_id/action/params_hash triple) → server executes; decline excluded (touches settleFault); execute unreachable by agent-bearer (I1). SHIPPED + deployed (#284-286) | shipped | normal-but-sensitive (money-adjacent) |
| [RFC-022](RFC-022-payment-provider-seam.md) | Payment Provider Seam — first-class `payment_session`+`payment_event` entities + `PaymentProvider` interface/registry so PSP + on-chain USDC become **pluggable**, wiring the dead `payment-rails.ts` seam. No real money / no custody / no settleOrder refactor (PR-2 observe-only; dispatch refactor isolated to gated PR-2b). Real PSP charge + on-chain USDC custody stay GATED (prerequisites, not code) | draft | normal-but-sensitive (money-path-adjacent) |
