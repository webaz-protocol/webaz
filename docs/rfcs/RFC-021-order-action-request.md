...
**部分唯一索引(P2-a 决策 Y · I5 幂等 · I2 无 blanket):同一 (order_id, order_action) 全程仅一条 active(pending 或 approved)。**
> **P2-a 锁语义(定案):** WHERE `status IN ('pending','approved')` —— **approved 占锁**:一个已批准但未执行(PR2 恒未执行)的请求会持续占用该 (order_id, action) 槽位,同 (order_id,action) 重复提交被唯一索引 + 服务端 `DUPLICATE_ACTION_REQUEST` 拒。**approved 后需显式撤销才能重提**;撤销机制本身不在本 PR 范围(后续 RFC/PR),本 PR 只落"锁语义 + 拒重复提交"。rejected/expired/executed(PR3)为终态,不占锁 → 允许事后重提。
```sql
