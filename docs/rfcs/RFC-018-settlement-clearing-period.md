...
- **PR2a (core commission mechanism) — MERGED #81**: `settleCommission` path ⑤ accrues to pending (not the wallet) with `matures_at`; maturation cron (`rewards-clearing-mature`) re-validates genuinely-closed → commission_records + wallet, CAS-atomic + idempotent + hold-on-dispute; `returns.ts` reverses linked pending rows proportionally; escrow-expire + all opt-out-escrow consumers filter `matures_at IS NULL`. (Option A: commission_records written at maturation.)
- **PR2b (reputation score split) — SKIPPED (moot)**: verified the codebase has **no completion-time reputation accrual** to defer — `users.reputation` (the verifier/arbitrator gate) is never written, `lifetime_score` is never written, and `recordRepEvent('order_completed')` is never fired. The RFC Motivation's *"reputation events fire on completion"* describes intent the code never implemented. Clearing defers things credited at completion; the only such thing is commission (PR2a). If reputation-on-genuine-sale is ever ADDED, it is a new feature and must use the clearing model from day one.
- **PR3 / PR3-UI (surfaces) — MERGED #82 / #83**: API (promoter/wallet/MCP/referral) + PWA UI render `清算中` clearing total vs settled.
- **PR4 (eligibility / sales_count / completion_count returned-aware) — this PR**: extend `genuineSalePredicate` (SSOT) to also exclude **fully-refunded-return** orders (return happens after `confirmed→completed`, so the confirmed-only predicate counted them); migrate verifier/arbitrator eligibility, task-progress, profile counts, completion_count backfill, and the `sales_count` subquery (4 route files) to the predicate; decrement the stored `completion_count` on a full return in `executeReturnRefund`. Partial refunds still count.

- PR: PR1 #80 · PR2a #81 · PR3 #82 · PR3-UI #83 · PR4 (this)
- Commit:
- Closes issue:
...
- 2026-06-27: **commission half landed** (PR1 #80 schema, PR2a #81 mechanism, PR3 #82 API surfaces, PR3-UI #83 PWA render). **Decision #1's reputation-score half found MOOT in code** (no completion-time reputation accrual exists: `users.reputation`/`lifetime_score` never written, `recordRepEvent('order_completed')` never fired) → PR2b skipped. **PR4** makes `genuineSalePredicate` (and the eligibility/sales_count/completion_count consumers) returns-aware — closing decision #1's "eligibility-count fix" follow-up.
