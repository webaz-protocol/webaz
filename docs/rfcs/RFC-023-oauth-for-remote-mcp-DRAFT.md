...
- **Flow**: Authorization Code + PKCE(S256). User is redirected to `webaz.xyz`, authenticates with **Passkey** (accountability root, unchanged), sees a **consent screen** listing the requested SAFE scopes + the client identity, approves → client receives a short-lived, audience-bound access token (+ refresh per §6-D2).
...
- **D3 · Client onboarding** — (a) **allowlist of a few known clients (Claude/ChatGPT/Cursor)** *(recommended v1; smallest surface)* vs (b) Client ID Metadata Documents *(MCP-preferred direction; recommended v2)* vs (c) full DCR (RFC 7591) *(widest surface; defer)*.
