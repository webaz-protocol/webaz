...
## 8. Security review — round 2 (Codex, 2026-07-14)

Round-1 fix was **necessary but incomplete**: it patched the three obvious credential resolvers, but `apiCall()` — the actual network chokepoint — reads the host `WEBAZ_API_KEY` module const *directly* as a fallback (`opts.apiKey || WEBAZ_API_KEY`), bypassing the patch. Bare/public-read `apiCall()` sites (e.g. task-proposal suggest, secondhand/trial/skill-market reads) would still attach the host key for an anonymous remote caller. Also flagged: `recentCalls` ring buffer is process-global (cross-request metadata).
