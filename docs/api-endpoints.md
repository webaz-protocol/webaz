...
| GET | `/api/agent-grants` | 🔐 |  | ── Read: the human's connected agents (no secrets). ── | src/pwa/routes/agent-grants.ts:263 |
...
| POST | `/api/agent-grants/:grant_id/revoke` | 🔐 |  | ── Revoke (online, one-click). ── | src/pwa/routes/agent-grants.ts:280 |
