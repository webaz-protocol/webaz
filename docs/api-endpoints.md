...
Auto-generated from `src/pwa/server.ts` + `src/pwa/routes/*.ts` (754 endpoints).
...
| GET | `/api/products/:id/shipping-options` |  |  | 公开读:买家下单前查配送范围。生效 = 单品覆盖 ?? 店铺默认;template=null → 不按地区计费(下单不要求选地区)。 | src/pwa/routes/shipping-templates.ts:64 |
