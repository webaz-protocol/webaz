...
|---|---|---|---|---|---|
| GET | `/.well-known/did.json` |  |  | 任何标准 DID resolver(Veramo / SpruceID / KILT / web5 ...)可 GET → 解出 issuer key → 验  | src/pwa/routes/public-utils.ts:411 |
...
| GET | `/.well-known/openai-apps-challenge` |  |  |  | src/pwa/routes/openai-apps-challenge.ts:22 |
| GET | `/.well-known/webaz-acp-feed.json` |  |  |  | src/pwa/routes/public-utils.ts:404 |
| GET | `/.well-known/webaz-capabilities.json` |  |  | 集成方 agent fetch 此端点即知"我要做的写需要声明哪个 scope / 哪些写无需 scope / 哪些读受约束"。 | src/pwa/routes/public-utils.ts:292 |
| GET | `/.well-known/webaz-economic.json` |  |  |  | src/pwa/routes/public-utils.ts:384 |
| GET | `/.well-known/webaz-entities.json` |  |  | RFC-011 §① — agent 可读实体字典(订单状态机 doc=code + 保守公开字段 + 可验证标注)。 | src/pwa/routes/public-utils.ts:302 |
| GET | `/.well-known/webaz-goals.json` |  |  | RFC-011 §① 目标索引 —— intent → action(②)+ endpoint + MCP 工具 + PWA 页(agent 自路由)。 | src/pwa/routes/public-utils.ts:312 |
| GET | `/.well-known/webaz-integration.json` |  |  | RFC-011 总入口 —— 集成方 agent 一次 fetch 拿到整份契约导航(按旅程组织,指向各维度 live 端点)。 | src/pwa/routes/public-utils.ts:346 |
| GET | `/.well-known/webaz-launch-pulse.json` |  |  |  | src/pwa/routes/public-utils.ts:280 |
| GET | `/.well-known/webaz-negative-space.json` |  |  |  | src/pwa/routes/public-utils.ts:392 |
| GET | `/.well-known/webaz-protocol.json` |  |  |  | src/pwa/routes/public-utils.ts:226 |
| GET | `/.well-known/webaz-verifiability.json` |  |  | RFC-011 §⑤ 可验证索引 —— "什么可验 + 怎么验"统一表(护照/锚/AP2/订单链),诚实分级。 | src/pwa/routes/public-utils.ts:356 |
...
| GET | `/api/admin/auditor` | 🔐 | 👑 |  | src/pwa/routes/admin-analytics.ts:101 |
...
| GET | `/api/admin/dashboard` | 🔐 | 👑 |  | src/pwa/routes/admin-analytics.ts:249 |
...
| GET | `/api/admin/finance/monthly` | 🔐 | 👑 |  | src/pwa/routes/admin-analytics.ts:108 |
...
| GET | `/api/admin/protocol-kpi` | 🔐 | 👑 |  | src/pwa/routes/admin-analytics.ts:161 |
...
| GET | `/api/admin/quota-applications` | 🔐 | 👑 | Admin | src/pwa/routes/seller-quota.ts:229 |
| POST | `/api/admin/quota-applications/:id/approve` | 🔐 | 👑 |  | src/pwa/routes/seller-quota.ts:242 |
| POST | `/api/admin/quota-applications/:id/reject` | 🔐 | 👑 |  | src/pwa/routes/seller-quota.ts:259 |
...
| GET | `/api/admin/rewards-health` | 🔐 | 👑 | 以及"在旧 major consent 上仍 opted-in"= 下次 auto_downgrade cron 的降级候选。 | src/pwa/routes/admin-analytics.ts:303 |
...
| GET | `/api/agent-grants` | 🔐 |  | "Connected agents" UI shows so a human can spot stale/unused or busy agents befo | src/pwa/routes/agent-grants.ts:1235 |
| POST | `/api/agent-grants` | 🔐 |  | agent delegation"的安全叙事。零消费方(前端/MCP/测试均不用),故降级为不可用,统一走 pairing。 | src/pwa/routes/agent-grants.ts:1224 |
| POST | `/api/agent-grants/:grant_id/revoke` | 🔐 |  | ── Revoke (online, one-click). ── | src/pwa/routes/agent-grants.ts:1268 |
| GET | `/api/agent-grants/connection` | 🎫 grant:read_public |  | address, or any other PII (E-node requirement). Backs webaz_connection_status. | src/pwa/routes/agent-grants.ts:217 |
| GET | `/api/agent-grants/my-permission-requests` |  |  | grant_id: an agent sees ONLY its own requests, never the human's other agents'.  | src/pwa/routes/agent-grants.ts:918 |
| POST | `/api/agent-grants/pair/:pairing_id/retrieve` |  |  | (pair 4) Agent retrieves the credential ONCE via PKCE verifier — UNAUTHENTICATED | src/pwa/routes/agent-grants.ts:1187 |
| GET | `/api/agent-grants/pair/:user_code` | 🔐 |  | (pair 2) Human reviews the server-generated consent — human-authenticated. | src/pwa/routes/agent-grants.ts:1119 |
| POST | `/api/agent-grants/pair/:user_code/approve` | 🔐 |  | (pair 3) Human approves — human-authenticated. Issues the grant (token_hash fill | src/pwa/routes/agent-grants.ts:1128 |
| POST | `/api/agent-grants/pair/:user_code/reject` | 🔐 |  | 拒绝是保护性动作,无需 Passkey(不签发任何凭证)。幂等:仅 pending 可拒。 | src/pwa/routes/agent-grants.ts:1176 |
| POST | `/api/agent-grants/pair/start` |  |  | (pair 1) Agent starts a pairing — UNAUTHENTICATED (agent has no credential yet). | src/pwa/routes/agent-grants.ts:1078 |
| GET | `/api/agent-grants/permission-requests` | 🔐 |  | GET list this human's PENDING permission requests (for #agent-approvals). Human- | src/pwa/routes/agent-grants.ts:876 |
| POST | `/api/agent-grants/permission-requests` |  |  | NOT grantable (they need a per-action live Passkey, not a persistent grant) → st | src/pwa/routes/agent-grants.ts:852 |
| POST | `/api/agent-grants/permission-requests/:id/approve` | 🔐 |  | POST approve — human-authed + live Passkey; expands the bound grant (union scope | src/pwa/routes/agent-grants.ts:934 |
| POST | `/api/agent-grants/permission-requests/:id/choose-payment` | 🔐 |  |  | src/pwa/routes/agent-grants.ts:1067 |
| GET | `/api/agent-grants/permission-requests/:id/payment-options` | 🔐 |  | GET: 列当前可选支付方式(卖家支持+资格通过)。POST: 选定 → 写草稿 rail/account + 重算 params_hash(旧 Passkey | src/pwa/routes/agent-grants.ts:1061 |
| POST | `/api/agent-grants/permission-requests/:id/reject` | 🔐 |  | POST reject — human-authed. Terminal 'rejected'; nothing is granted. | src/pwa/routes/agent-grants.ts:1042 |
...
| GET | `/api/agent/acp-feed` |  |  |  | src/pwa/routes/public-utils.ts:405 |
...
| GET | `/api/agent/approval-requests/:id` | 🎫 grant:approval_requests_read |  |  | src/pwa/routes/agent-grants.ts:632 |
| GET | `/api/agent/buyer/orders` | 🎫 grant:buyer_orders_read_minimal |  | recipient_code 连取都不取(I6 同强度)。纯只读,零执行、零资金 —— 买家写动作(place_order 等)仍 RISK 硬拒。 | src/pwa/routes/agent-grants.ts:283 |
| GET | `/api/agent/buyer/orders/:id` | 🎫 grant:buyer_orders_read_minimal |  |  | src/pwa/routes/agent-grants.ts:316 |
| GET | `/api/agent/buyer/orders/:id/case-draft` | 🎫 grant:buyer_case_prepare |  |  | src/pwa/routes/agent-grants.ts:640 |
| GET | `/api/agent/buyer/orders/:id/full` | 🎫 grant:buyer_orders_read |  | RFC-026 PR-3 — 订单全量只读(safe scope buyer_orders_read;时间线/条款快照/物流/截止/退款/动作面;零 PII) | src/pwa/routes/agent-grants.ts:613 |
| GET | `/api/agent/capabilities` |  |  |  | src/pwa/routes/public-utils.ts:296 |
| GET | `/api/agent/categories` |  |  |  | src/pwa/routes/agent-grants.ts:340 |
| GET | `/api/agent/changes` |  |  | 指纹由 tests/test-contract-fingerprint.ts + docs/CONTRACT-LOCK.json 守住(静默改契约不可 merg | src/pwa/routes/public-utils.ts:340 |
| POST | `/api/agent/discover` | 🎫 grant:buyer_discover |  |  | src/pwa/routes/agent-grants.ts:348 |
| GET | `/api/agent/economic-participation` |  |  |  | src/pwa/routes/public-utils.ts:385 |
| GET | `/api/agent/entities` |  |  |  | src/pwa/routes/public-utils.ts:306 |
| GET | `/api/agent/events` | 🔐 |  | 结构性事件 + 哈希链字段(验链防篡改),完整 payload 仍走 party-gated /chain。 | src/pwa/routes/orders-read.ts:163 |
| GET | `/api/agent/goals` |  |  |  | src/pwa/routes/public-utils.ts:332 |
| GET | `/api/agent/integration` |  |  |  | src/pwa/routes/public-utils.ts:350 |
| GET | `/api/agent/negative-space` |  |  |  | src/pwa/routes/public-utils.ts:393 |
| POST | `/api/agent/order-draft` | 🎫 grant:draft_order |  | 草稿不可变(无 update),cancel 终态幂等安全;get/list 仅本人。提交/批准/建单全在 PR-5a。 | src/pwa/routes/agent-grants.ts:547 |
| GET | `/api/agent/order-drafts` | 🎫 grant:draft_order |  |  | src/pwa/routes/agent-grants.ts:559 |
| GET | `/api/agent/order-drafts/:id` | 🎫 grant:draft_order |  |  | src/pwa/routes/agent-grants.ts:563 |
| POST | `/api/agent/order-drafts/:id/cancel` | 🎫 grant:draft_order |  |  | src/pwa/routes/agent-grants.ts:553 |
| POST | `/api/agent/order-drafts/:id/submit` | 🎫 grant:order_submit_request |  |  | src/pwa/routes/agent-grants.ts:647 |
| GET | `/api/agent/orders` | 🎫 grant:seller_orders_read_minimal |  | 纯只读,无任何执行(order_action_request 在 PR2/PR3 才有提交/执行)。 | src/pwa/routes/agent-grants.ts:263 |
| GET | `/api/agent/orders/:id` | 🎫 grant:seller_orders_read_minimal |  |  | src/pwa/routes/agent-grants.ts:270 |
| POST | `/api/agent/orders/:id/buyer-action-request` | 🎫 grant:buyer_action_request |  | RFC-026 PR-6 — 买家动作请求(confirm_receipt/cancel/request_return;提交-审批-回环执行) | src/pwa/routes/agent-grants.ts:576 |
| GET | `/api/agent/orders/:id/chat` | 🎫 grant:order_chat_read |  | RFC-026 PR-4 — 订单上下文聊天(仅订单双方;发送回环走生产反诈/限频;无自由私信) | src/pwa/routes/agent-grants.ts:598 |
| POST | `/api/agent/orders/:id/chat` | 🎫 grant:order_chat_send |  |  | src/pwa/routes/agent-grants.ts:604 |
| POST | `/api/agent/orders/:orderId/action-request` | 🎫 grant:order_action_request |  | 同 (order_id,action) 双 pending 被唯一索引拒。执行(accept/ship)在 PR3 经人 Passkey 批准后由服务端跑。 | src/pwa/routes/agent-grants.ts:679 |
| POST | `/api/agent/quote` | 🎫 grant:price_quote |  | subject 恒 = grant human(agent 无法传 human_id/代表他人)。 | src/pwa/routes/agent-grants.ts:537 |
| GET | `/api/agent/request-readiness` |  |  | 放在 categories(同样无鉴权)之前 —— gen-openapi 的「后 8 行探 requireAgentGrantScope」启发式才不会误把下方 | src/pwa/routes/agent-grants.ts:338 |
| GET | `/api/agent/seller/products` | 🎫 grant:seller_products_read |  | The consumption (allow AND the permission_required deny) is audited by the middl | src/pwa/routes/agent-grants.ts:251 |
| POST | `/api/agent/seller/products` | 🎫 grant:seller_product_draft |  |  | src/pwa/routes/agent-grants.ts:695 |
| PATCH | `/api/agent/seller/products/:id/draft` | 🎫 grant:seller_product_draft |  |  | src/pwa/routes/agent-grants.ts:724 |
| POST | `/api/agent/seller/products/:id/images` | 🎫 grant:seller_product_draft |  |  | src/pwa/routes/agent-grants.ts:777 |
| GET | `/api/agent/verifiability` |  |  |  | src/pwa/routes/public-utils.ts:360 |
| GET | `/api/agent/wallet` | 🎫 grant:wallet_read_minimal |  | RFC-026 PR-3 — 钱包最小只读(safe scope wallet_read_minimal;OAuth 钱包面永远只读) | src/pwa/routes/agent-grants.ts:621 |
...
| GET | `/api/direct-receive/deferral` | 🔐 |  | GET /api/direct-receive/deferral — 卖家本人缓交状态:最新一条申请(脱敏:不含 admin 身份)+ 是否当前生效(activ | src/pwa/routes/direct-pay-availability.ts:76 |
| POST | `/api/direct-receive/deferral` | 🔐 |  | POST /api/direct-receive/deferral — 卖家申请缓交。helper 强制:单一活跃、periodDays 正整数、id 唯一。 | src/pwa/routes/direct-pay-availability.ts:66 |
...
| GET | `/api/direct-receive/my-fee-account` | 🔐 |  | 仅本人(requireSeller),买家拿不到;只读、不碰任何资金动作。供 seller fee center 展示。 | src/pwa/routes/direct-pay-availability.ts:89 |
...
| POST | `/api/direct-receive/product-verification` | 🔐 |  | POST /api/direct-receive/product-verification — 卖家为某产品申领验证码(单一活跃 per product)。 | src/pwa/routes/direct-pay-availability.ts:109 |
| PUT | `/api/direct-receive/product-verification` | 🔐 |  | PUT /api/direct-receive/product-verification — 卖家为某产品提交外部商品链接(链接仅存储,WebAZ 不抓取)。 | src/pwa/routes/direct-pay-availability.ts:119 |
| GET | `/api/direct-receive/product-verifications` | 🔐 |  | GET /api/direct-receive/product-verifications — 卖家本人所有产品的认证状态(逐产品)。 | src/pwa/routes/direct-pay-availability.ts:130 |
| GET | `/api/direct-receive/readiness` | 🔐 |  | 绝不下发 raw blocker / KYB·制裁·AML 分项(见 sellerDirectPayReadinessView)。只读 self(auth 用户 | src/pwa/routes/direct-pay-availability.ts:56 |
...
| GET | `/api/direct-receive/store-verification` | 🔐 |  | GET /api/direct-receive/store-verification — 卖家本人店铺认证状态(脱敏 DTO,含豁免位)。 | src/pwa/routes/direct-pay-availability.ts:158 |
| POST | `/api/direct-receive/store-verification` | 🔐 |  | POST /api/direct-receive/store-verification — 卖家申领店铺验证码(单一活跃 per seller)。 | src/pwa/routes/direct-pay-availability.ts:139 |
...
| GET | `/api/editor-picks` |  |  |  | src/pwa/routes/public-utils.ts:448 |
...
| POST | `/api/error-report` |  |  |  | src/pwa/routes/public-utils.ts:528 |
...
| GET | `/api/governance/onboarding-stats` |  |  | 无 auth — agent / 用户 / 第三方都可读;不暴露 PII | src/pwa/routes/public-utils.ts:476 |
...
| GET | `/api/health` |  |  |  | src/pwa/routes/public-utils.ts:54 |
...
| GET | `/api/launch-pulse` |  |  |  | src/pwa/routes/public-utils.ts:284 |
...
| POST | `/api/orders/:id/fault-refund/decline` | 🔐 |  |  | src/pwa/routes/direct-fault-refund.ts:65 |
| POST | `/api/orders/:id/fault-refund/escalate` | 🔐 |  |  | src/pwa/routes/direct-fault-refund.ts:113 |
...
| GET | `/api/protocol-status` |  |  |  | src/pwa/routes/public-utils.ts:230 |
...
| POST | `/api/seller/apply-quota-increase` | 🔐 |  |  | src/pwa/routes/seller-quota.ts:197 |
...
| POST | `/api/seller/withdraw-quota-application` | 🔐 |  |  | src/pwa/routes/seller-quota.ts:220 |
...
| GET | `/api/sellers/me/return-stats` | 🔐 |  | 卖家退货仪表盘 | src/pwa/routes/analytics.ts:289 |
...
| GET | `/api/system-flags` |  |  |  | src/pwa/routes/public-utils.ts:103 |
...
| GET | `/api/usdc-escrow/payout-addresses` |  |  |  | src/pwa/routes/usdc-payout-address.ts:35 |
| POST | `/api/usdc-escrow/payout-addresses` |  |  |  | src/pwa/routes/usdc-payout-address.ts:40 |
| POST | `/api/usdc-escrow/payout-addresses/:id/retire` |  |  |  | src/pwa/routes/usdc-payout-address.ts:47 |
