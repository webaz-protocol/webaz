...
Auto-generated from `src/pwa/server.ts` + `src/pwa/routes/*.ts` (830 endpoints).
...
| GET | `/api/orders/:id/usdc-escrow/status` | 🔐 |  | GET /api/orders/:id/usdc-escrow/status —— 买家或卖家:轮询存入/释放可见性(B6b stepper)。 | src/pwa/routes/usdc-escrow.ts:260 |
| POST | `/api/orders/:id/usdc-escrow/voucher` | 🔐 |  | POST /api/orders/:id/usdc-escrow/voucher —— 买家:签发一次性 EIP-712 存款授权。 | src/pwa/routes/usdc-escrow.ts:151 |
