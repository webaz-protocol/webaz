...
## BUG-08 Phase-3A final closure (4 remaining items)
3 commits (7dbcb38 再买一份 end-to-end chain · 21c101e trace wiring + tools/list budget reclamation · a1db194 exec/restart/trace tests). §一 再买一份 quote→draft→submit DIRECT_TOOL chain (one instance threaded, distinct keys, fail-stop, single-flight); §二 zero-PII trace wired component→handler→route→table (bridge_type standard/legacy; observation-only); §三 execution re-validation proven (no double-order/charge on expiry/price/stock/delisted/region/address; duplicate Passkey → one order); §四 restart recovery + cross-connection uniqueness. Reclaimed the tools/list token budget (was red at the BUG-08 HEAD). No money/state/ETA/schema-v2 change. Live-host items in KNOWN_LIMITATIONS.
