...
|---|---|---|
| `shopping_v1` (1) | stable public discovery plugin contract: anonymous product search · exact anchor/link lookup · product details/cards | `/mcp?surface=shopping_v1` |
...
The `buyer` and `seller` surfaces affect `tools/list` visibility only; their existing call-time authorization
remains authoritative. `shopping_v1` is different: it is a reviewed public distribution boundary, so both
`tools/list` and `tools/call` enforce its allowlist. Cached calls to tools outside that release are rejected.
stdio (`npx -y @seasonkoh/webaz`) and the explicit `full` surface retain the complete tool set.

`shopping_v1` is intentionally versioned and frozen for public review. Its sole advertised tool is
`webaz_search`; changing that set or its published metadata requires a new reviewed plugin version.
The first public release is anonymous and read-only: it does not advertise OAuth, order creation,
private orders, payments, seller operations, or account data. Handler fixes and backward-compatible
live results continue to share the canonical implementation used by the other surfaces.

Public results are also restricted by the deployment allowlist
`WEBAZ_PUBLIC_COMMERCE_ALLOWED_PRODUCT_IDS` (a comma-separated list or JSON array, maximum 200
`prd_...` IDs). Missing, empty, malformed, or oversized configuration fails closed to zero products.
An allowlisted product must still be active, in stock, retail, and clear the existing seller-pause and
external-link revocation checks. Configure this Railway variable before public-review submission; it
does not change the catalog visible through the full, buyer, or seller MCP surfaces.

**QuoteAndApproval (PR-5):** `webaz_quote_order` / `webaz_order_draft` / `webaz_submit_order_request` declare `openai/outputTemplate` → `ui://widget/webaz-quote-approval.html` — one component rendering quote → draft → Passkey-approval states with USDC pricing + display-only fiat estimates (`fiat_estimate` with `≈`, `estimated`, `stale`), rail-honesty notes (simulated escrow ≠ real USDC custody; Direct Pay = WebAZ holds no principal), the three no-charge/no-stock/Passkey disclosures, and an explicit duplicate-purchase warning card. Consumer projections: `webaz.order_quote/order_draft/order_approval.model.v1` — **WAZ is never a consumer-facing currency**; protocol-recorded units stay server-side.

**OrderTimeline (PR-6):** `webaz_buyer_orders` declares `openai/outputTemplate` → `ui://widget/webaz-order-timeline.html`. Full order reads (`{order_id, full:true}`) project to `webaz.order_timeline.model.v1`: USDC price + display-only fiat estimate, status labels from the protocol state machine (single truth source), the state-history timeline, logistics/tracking, and a rail badge — Direct Pay orders read "直付(WebAZ 不托管本金)", every other rail reads "模拟托管测试订单 — 不代表真实 USDC 或法币托管". Refund display is rail-honest: Direct Pay refunds show the protocol-recorded responsibility outcome (WebAZ held no principal; the actual refund completes between buyer and seller — never "WebAZ has refunded"); simulated-escrow refunds are labeled as simulated releases, not real funds flows. Deadlines ship as ISO timestamps with a render-in-viewer-timezone note (the widget renders local time). The order **list** (7-key contract) and `updated_since` → `up_to_date` incremental responses are unchanged; the minimal single-order response now additionally carries a top-level `schema_version` (additive, contract v29 — the 7-key `order` projection itself is unchanged). `available_actions` is server-authoritative (action + executor only); contact-seller returns to the conversation flow bound to `webaz_order_chat` + the order id.

**Standard MCP Apps (PR-A, cross-host):** the five UI tools dual-publish component metadata — standard `_meta.ui.resourceUri` (SEP-1865, spec 2026-01-26) **and** the legacy `openai/outputTemplate` — pointing at separate, stable resources: `ui://widget/<name>-mcp.html` (`text/html;profile=mcp-app`, `_meta.ui.csp` deny-by-default with four empty domain arrays, no `ui.domain`) and the unchanged `ui://widget/<name>.html` (`text/html+skybridge`, `openai/widgetCSP`/`openai/widgetDomain` untouched). Both resources share the same render body; only the boot differs — the standard variant speaks the `ui/*` postMessage JSON-RPC bridge (three-step `ui/initialize` handshake, results via `ui/notifications/tool-result`, `ui/open-link`, `ui/message`), falls back to `window.openai` on handshake timeout, and degrades to read-only when neither bridge exists. Selection is capability-probed — no host-name or User-Agent branching. Tool `visibility` is mapped per tool (`webaz_quote_order` is `['model']` only — the widget never calls it); `openai/widgetAccessible: true` marks the four widget-callable tools (search/draft/submit/buyer_orders). Widget `openExternal` is origin-locked to `https://webaz.xyz` on both paths (parsed-URL comparison; `javascript:`/`data:`/protocol-relative/userinfo forms rejected); the contact-seller entry uses `sendFollowUpMessage` with a `sendFollowupTurn` fallback (single-send guarded). Visibility hints affect UI hosts only — server-side authorization (OAuth scope, Passkey, idempotency) is unchanged.

**MCP App (ChatGPT progressive enhancement):** `webaz_search` declares `openai/outputTemplate` → `ui://widget/webaz-products.html` — hosts that render MCP Apps (verified: ChatGPT) show the **ProductResults** component (cards, local sort/expand/compare — local interactions never call the model; the quote entry returns to the conversation flow and every economic action still ends at the webaz.xyz Passkey). Hosts without Apps support (verified: Claude) see the standard structuredContent + actionable summary — the standard fields remain the primary path.
